...
apiClient.interceptors.request.use(
  (config) => {
    ...
    if (url.includes("/auth")) serviceKey = "auth";
    else if (url.includes("/patients")) serviceKey = "patients";
    else if (url.includes("/clinical")) serviceKey = "clinical";
    else if (url.includes("/compliance")) serviceKey = "compliance";
    else if (url.includes("/referrals")) serviceKey = "referrals";
    else if (url.includes("/claims")) serviceKey = "claims";
    else if (url.includes("/revenue")) serviceKey = "revenue";
    else if (url.includes("/payments")) serviceKey = "payments";
    else if (url.includes("/denials")) serviceKey = "denials";
    else if (url.includes("/reconciliation")) serviceKey = "reconciliation";
    else if (url.includes("/authorizations")) serviceKey = "authorizations";
    else if (url.includes("/daman")) serviceKey = "daman";

    const breaker = circuitBreakers[serviceKey as keyof typeof circuitBreakers];

    if (!breaker.isAllowed()) {
      return Promise.reject(
        new Error(
          `Service ${serviceKey} is currently unavailable. Circuit breaker is ${breaker.getState()}`,
        ),
      );
    }
    ...
    if (error.message === "Network Error" || !navigator.onLine) {
      ...
      if (
        ...
        try {
          ...
          console.error(
            "Failed to queue request for offline mode:",
            queueError,
          );
        }
      }
    ...
    if (
      ...
      try {
        ...
        const response = await apiClient.post(
          `${SERVICE_ENDPOINTS.auth}/refresh-token`,
          {
            refreshToken,
          },
        );
      ...
      if (error.response) {
        ...
        const breaker =
          circuitBreakers[serviceKey as keyof typeof circuitBreakers];
        const isOpen = breaker.onFailure();

        if (isOpen) {
          console.warn(
            `Circuit breaker for ${serviceKey} is now OPEN due to multiple failures`,
          );
        }
      }
    }
...
export class ApiService {
  ...
  static async validateAdhicsCompliance(data: any): Promise<any> {
    try {
  ...
  static async submitDamanAuthorization(authorizationData: any): Promise<any> {
    try {
      ...
      try {
        ...
        if (!validation.isValid) {
          ...
          throw new Error(
            `Data validation failed: ${validation.errors?.join(", ") || "Unknown validation error"}`,
          );
        }
      ...
      try {
        ...
        if (!finalValidation.isValid) {
          ...
          throw new Error(
            `Final validation failed: ${finalValidation.errors?.join(", ") || "Unknown validation error"}`,
          );
        }
      ...
      const response = await this.post(
        `${SERVICE_ENDPOINTS.authorizations}/daman/submit`,
        encryptedData,
        {
          headers: {
      ...
      await this.logDamanAuthorizationSubmission(
        response.id,
        validatedData,
        response,
      );
  ...
  static async getDamanAuthorizationStatus(
    referenceNumber: string,
  ): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.authorizations}/daman/status/${referenceNumber}`,
      );
    } catch (error) {
      console.error(
        `Error fetching status for authorization ${referenceNumber}:`,
        error,
      );
      throw error;
    }
  ...
  static async getDamanAuthorizationDetails(
    authorizationId: string,
  ): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.authorizations}/daman/${authorizationId}`,
      );
    } catch (error) {
      console.error(
        `Error fetching details for authorization ${authorizationId}:`,
        error,
      );
      throw error;
    }
  ...
  static async uploadAdditionalDocuments(
    authorizationId: string,
    formData: FormData,
  ): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.authorizations}/daman/${authorizationId}/documents`,
        formData,
        { headers: { "Content-Type": "multipart/form-data" } },
      );
    } catch (error) {
      console.error(
        `Error uploading additional documents for authorization ${authorizationId}:`,
        error,
      );
      throw error;
    }
  ...
  static async registerForAuthorizationWebhooks(
    authorizationId: string,
    webhookData: any,
  ): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.authorizations}/webhooks/register`,
        webhookData,
      );
    } catch (error) {
      console.error(
        `Error registering webhooks for authorization ${authorizationId}:`,
        error,
      );
      throw error;
    }
  ...
  private static validateDamanAuthorizationData(authorizationData: any): void {
    ...
    if (
      !Array.isArray(authorizationData.documents) ||
    ...
    const providedDocuments = authorizationData.documents.map(
      (doc: any) => doc.type || doc,
    );
    const missingDocuments = requiredDocuments.filter(
      (doc) =>
        !providedDocuments.includes(doc) &&
    ...
    if (
      !authorizationData.digitalSignatures?.patientSignature ||
      !authorizationData.digitalSignatures?.providerSignature ||
      !authorizationData.digitalSignatures?.contactPersonSignature
    ) {
      throw new Error(
        "Patient, provider, and contact person signatures are required",
      );
    }

    // Enhanced clinical justification validation
    if (
      !authorizationData.clinicalJustification ||
      authorizationData.clinicalJustification.length < 100
    ) {
      throw new Error(
        "Clinical justification must be at least 100 characters long and include detailed medical necessity",
      );
    }

    // Service codes validation against Daman 2025 standards
    if (
      !Array.isArray(authorizationData.requestedServices) ||
    ...
    authorizationData.requestedServices.forEach(
      (service: any, index: number) => {
        if (deprecatedCodesList.includes(service.serviceCode)) {
          const deprecatedInfo = deprecatedCodes[service.serviceCode];
          throw new Error(
            `Deprecated service code at index ${index}: ${service.serviceCode} (${deprecatedInfo.description}). Please use replacement code: ${deprecatedInfo.replacement}. Deprecated codes are not billable since June 1, 2024.`,
          );
        }
        if (!validServiceCodes.includes(service.serviceCode)) {
          throw new Error(
            `Invalid service code at index ${index}: ${service.serviceCode}. Valid codes for 2025: ${validServiceCodes.join(", ")}`,
          );
        }
        ...
        if (
          ...
          throw new Error(
            `Incorrect pricing for service code ${service.serviceCode} at index ${index}. Expected: AED ${expectedPricing[service.serviceCode]}, Found: AED ${service.unitPrice}`,
          );
        }
      },
    );
    ...
    if (
      submissionTime > deadlineTime &&
      submissionTime.getDate() === deadlineTime.getDate()
    ) {
      const nextDeadline = new Date(deadlineTime);
      nextDeadline.setDate(nextDeadline.getDate() + 1);
      const hoursLate = Math.floor(
        (submissionTime.getTime() - deadlineTime.getTime()) / (1000 * 60 * 60),
      );
      const minutesLate = Math.floor(
        ((submissionTime.getTime() - deadlineTime.getTime()) %
          (1000 * 60 * 60)) /
          (1000 * 60),
      );
      ...
      if (hoursLate >= 4) {
        throw new Error(
          `Critical submission delay: ${hoursLate}h ${minutesLate}m after 8:00 AM UAE time deadline. Submission rejected. Next deadline: ${nextDeadline.toLocaleString()}`,
        );
      } else {
        console.warn(
          `Late submission warning: ${hoursLate}h ${minutesLate}m after 8:00 AM UAE time deadline. Escalation approval required.`,
        );
      }
    }
  ...
  private static isDocumentRequired(
    documentType: string,
    authorizationData: any,
  ): boolean {
    switch (documentType) {
      case "wheelchair-pre-approval":
        return authorizationData.requestedServices?.some(
          (service: any) =>
            service.serviceCode?.includes("wheelchair") ||
  ...
  private static validateMSCSpecificRequirements(authorizationData: any): void {
    ...
    if (authorizationData.initialVisitDate) {
      ...
      if (daysDifference > 90) {
        throw new Error(
          "Initial visit must be within 90 days of ATC effective date",
        );
      }
    }
    ...
    if (
      authorizationData.treatmentPeriod &&
  ...
  private static validateMSCCompliance(authorizationData: any): void {
    if (authorizationData.policyType !== "MSC") return;

    // ATC validity and revision requirements
    if (
      !authorizationData.atcValidityRevision &&
      authorizationData.initialVisitDate
    ) {
      throw new Error(
        "ATC validity revision required when initial visit date is provided",
      );
    }
    ...
    if (authorizationData.treatmentStartDate) {
      ...
      const daysDifference =
        (treatmentStart.getTime() - atcEffective.getTime()) /
        (1000 * 60 * 60 * 24);

      if (daysDifference > 90) {
        throw new Error(
          "Treatment must start within 90 days of ATC effective date",
        );
      }
    }
  ...
  // Provider authentication validation
  private static async validateProviderAuthentication(
    providerId: string,
  ): Promise<void> {
    try {
      const providerAuth = await this.get(
        `/api/providers/${providerId}/authentication`,
      );
  ...
  private static async encryptDamanSensitiveData(data: any): Promise<any> {
  ...
  private static async logDamanAuthorizationSubmission(
    id: string,
    requestData: any,
    responseData: any,
  ): Promise<void> {
  ...
  private static async updateComplianceMetrics(
    eventType: string,
    data: any,
  ): Promise<void> {
    try {
  ...
  private static async logComplianceViolation(
  ...
  private static calculateComplianceLevel(data: any): string {
    let score = 0;
    const maxScore = 10;

    // Check various compliance factors
    if (data.digitalSignatures?.patientSignature) score += 2;
    if (data.digitalSignatures?.providerSignature) score += 2;
    if (data.letterOfAppointment?.valid) score += 2;
    if (data.clinicalJustification?.length >= 100) score += 1;
    if (data.documents?.length >= 5) score += 1;
    if (data.contactPersonDetails?.email?.endsWith(".ae")) score += 1;
    if (data.faceToFaceAssessment?.completed) score += 1;
  ...
  private static sanitizeSubmissionData(data: any): any {
    try {
  ...
  private static sanitizeObjectStrings(obj: any, depth = 0): void {
  ...
  private static removeProblematicValues(obj: any, depth = 0): void {
    if (depth > 50) return; // Prevent infinite recursion
  ...
  private static validateObjectStructure(obj: any): void {
    try {
      ...
      throw new Error(
        `Object structure validation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private static sanitizeForAudit(data: any): any {
    try {
      const sanitized = JSON.parse(JsonValidator.safeStringify(data));
  ...
  private static async logAuthorizationSubmission(
    id: string,
    requestData: any,
    responseData: any,
  ): Promise<void> {
  ...
  // Get claim status
  static async getClaimStatus(claimNumber: string): Promise<any> {
    try {
      return await this.get(
  ...
  // Get claim details
  static async getClaimDetails(claimId: string): Promise<any> {
    try {
  ...
  static async recordPayment(paymentData: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.payments}/record`,
        paymentData,
      );
    } catch (error) {
  ...
  static async reconcilePayment(reconciliationData: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.reconciliation}/reconcile`,
        reconciliationData,
      );
    } catch (error) {
  ...
  static async getPaymentHistory(claimId: string): Promise<any> {
    try {
      return await this.get(`${SERVICE_ENDPOINTS.payments}/history/${claimId}`);
    } catch (error) {
      console.error(
        `Error fetching payment history for claim ${claimId}:`,
        error,
      );
      throw error;
    }
  ...
  static async recordDenial(denialData: any): Promise<any> {
    try {
  ...
  static async submitAppeal(appealData: any): Promise<any> {
    try {
  ...
  static async getDenialHistory(claimId: string): Promise<any> {
    try {
      return await this.get(`${SERVICE_ENDPOINTS.denials}/history/${claimId}`);
    } catch (error) {
      console.error(
        `Error fetching denial history for claim ${claimId}:`,
        error,
      );
      throw error;
    }
  ...
  static async generateRevenueReport(reportParams: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.revenue}/reports/generate`,
        reportParams,
      );
    } catch (error) {
  ...
  static async getAccountsReceivableAging(params?: any): Promise<any> {
    try {
  ...
  static async getRevenueAnalytics(params?: any): Promise<any> {
    try {
  ...
  static async getDenialAnalytics(params?: any): Promise<any> {
    try {
  ...
  // Validate claim data
  private static validateClaimData(claimData: any): void {
    ...
    if (
      !Array.isArray(claimData.serviceLines) ||
    ...
    claimData.serviceLines.forEach((line: any, index: number) => {
      ...
      const missingLineFields = requiredLineFields.filter(
        (field) => !line[field],
      );

      if (missingLineFields.length > 0) {
        throw new Error(
          `Service line ${index + 1} is missing required fields: ${missingLineFields.join(", ")}`,
        );
      }

      if (line.quantity <= 0 || line.unitPrice <= 0) {
        throw new Error(
          `Service line ${index + 1} has invalid quantity or unit price`,
        );
      }
    });
    ...
    const missingDocuments = requiredDocuments.filter(
      (doc) => !claimData.documents.includes(doc),
    );
  ...
  private static async logClaimSubmission(
    id: string,
    requestData: any,
    responseData: any,
  ): Promise<void> {
  ...
  static async getDamanSystemStatus(): Promise<any> {
    try {
  ...
  static async verifyPatientEligibility(patientData: any): Promise<any> {
    try {
      // Validate patient data before sending
      this.validatePatientEligibilityData(patientData);

      return await this.post(
        `${SERVICE_ENDPOINTS.daman}/eligibility/verify`,
        patientData,
      );
    } catch (error) {
  ...
  static async syncAuthorizationStatus(authorizationId: string): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.daman}/authorization/${authorizationId}/sync`,
      );
    } catch (error) {
      console.error(
        `Error syncing authorization status ${authorizationId}:`,
        error,
      );
      throw error;
    }
  ...
  static async handleDamanWebhook(webhookData: any): Promise<any> {
    try {
      // Validate webhook signature and data
      this.validateWebhookData(webhookData);

      return await this.post(
        `${SERVICE_ENDPOINTS.daman}/webhooks/handle`,
        webhookData,
      );
    } catch (error) {
  ...
  static async getOpenJetProviderServices(providerId: string): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.openjet}/provider/${providerId}/services`,
      );
    } catch (error) {
      console.error(
        `Error fetching OpenJet provider services for ${providerId}:`,
        error,
      );
      throw error;
    }
  ...
  static async submitOpenJetQuery(queryData: any): Promise<any> {
    try {
      this.validateOpenJetQueryData(queryData);

      return await this.post(
        `${SERVICE_ENDPOINTS.openjet}/queries/submit`,
        queryData,
      );
    } catch (error) {
  ...
  static async routeServiceRequest(requestData: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.openjet}/requests/route`,
        requestData,
      );
    } catch (error) {
  ...
  static async syncServiceRequestStatus(requestId: string): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.openjet}/requests/${requestId}/status`,
      );
    } catch (error) {
      console.error(
        `Error syncing service request status ${requestId}:`,
        error,
      );
      throw error;
    }
  ...
  static async validateLetterOfAppointment(appointmentData: any): Promise<any> {
    try {
      this.validateAppointmentLetterData(appointmentData);

      return await this.post(
        `${SERVICE_ENDPOINTS.auth}/appointment/validate`,
        appointmentData,
      );
    } catch (error) {
  ...
  static async manageDesignatedContact(contactData: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.auth}/contacts/manage`,
        contactData,
      );
    } catch (error) {
  ...
  static async validateDamanAccess(
    userId: string,
    resource: string,
  ): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.auth}/daman/access/${userId}/${resource}`,
      );
    } catch (error) {
  ...
  static async submitAuditLogEntry(auditData: any): Promise<any> {
    try {
      this.validateAuditLogData(auditData);

      return await this.post(
        `${SERVICE_ENDPOINTS.compliance}/audit/log`,
        auditData,
      );
    } catch (error) {
  ...
  static async generateComplianceReport(reportParams: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.compliance}/reports/generate`,
        reportParams,
      );
    } catch (error) {
  ...
  private static validatePatientEligibilityData(data: any): void {
    const requiredFields = ["emiratesId", "membershipNumber", "dateOfBirth"];
    const missingFields = requiredFields.filter((field) => !data[field]);

    if (missingFields.length > 0) {
      throw new Error(
        `Missing required fields for eligibility verification: ${missingFields.join(", ")}`,
      );
    }
  }

  private static validateWebhookData(data: any): void {
  ...
  private static validateOpenJetQueryData(data: any): void {
    const requiredFields = ["providerId", "queryType", "queryData"];
    const missingFields = requiredFields.filter((field) => !data[field]);

    if (missingFields.length > 0) {
      throw new Error(
        `Missing required fields for OpenJet query: ${missingFields.join(", ")}`,
      );
    }
  }

  private static validateAppointmentLetterData(data: any): void {
    const requiredFields = [
      "providerId",
      "appointmentDocument",
      "contactPersonDetails",
    ...
    if (missingFields.length > 0) {
      throw new Error(
        `Missing required fields for appointment letter: ${missingFields.join(", ")}`,
      );
    }
  }

  private static validateAuditLogData(data: any): void {
    const requiredFields = ["userId", "action", "resource", "timestamp"];
    const missingFields = requiredFields.filter((field) => !data[field]);

    if (missingFields.length > 0) {
      throw new Error(
        `Missing required fields for audit log: ${missingFields.join(", ")}`,
      );
    }
  }
  ...
  static async getPaymentReconciliation(params?: any): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.reconciliation}/payments`,
        params,
      );
    } catch (error) {
  ...
  static async processPaymentReconciliation(
    reconciliationData: any,
  ): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.reconciliation}/process`,
        reconciliationData,
      );
    } catch (error) {
  ...
  static async getClaimAgingReport(params?: any): Promise<any> {
    try {
  ...
  static async getPayerPerformanceAnalytics(params?: any): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.revenue}/payer-performance`,
        params,
      );
    } catch (error) {
  ...
  static async generateCashFlowProjection(params?: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.revenue}/cash-flow-projection`,
        params,
      );
    } catch (error) {
  ...
  static async getKPIDashboard(params?: any): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.revenue}/kpi-dashboard`,
        params,
      );
    } catch (error) {
  ...
  // Submit batch claims
  static async submitBatchClaims(claimsData: any[]): Promise<any> {
    try {
  ...
  static async getClaimValidationResults(claimId: string): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.claims}/${claimId}/validation`,
      );
    } catch (error) {
      console.error(
        `Error fetching validation results for claim ${claimId}:`,
        error,
      );
      throw error;
    }
  ...
  // Get appeal status
  static async getAppealStatus(appealId: string): Promise<any> {
    try {
  ...
  static async updateAppealStatus(
    ...
    try {
      return await this.patch(
        `${SERVICE_ENDPOINTS.denials}/appeals/${appealId}/status`,
        {
          status,
          notes,
          updatedAt: new Date().toISOString(),
  ...
  static async getRevenueCycleMetrics(params?: any): Promise<any> {
    try {
      return await this.get(
        `${SERVICE_ENDPOINTS.revenue}/cycle-metrics`,
        params,
      );
    } catch (error) {
  ...
  // Export revenue data
  static async exportRevenueData(params: any): Promise<any> {
    try {
  ...
  // Get payment trends
  static async getPaymentTrends(params?: any): Promise<any> {
    try {
  ...
  // Get denial trends
  static async getDenialTrends(params?: any): Promise<any> {
    try {
  ...
  static async scheduleAutomatedReport(reportConfig: any): Promise<any> {
    try {
      return await this.post(
        `${SERVICE_ENDPOINTS.revenue}/schedule-report`,
        reportConfig,
      );
    } catch (error) {
