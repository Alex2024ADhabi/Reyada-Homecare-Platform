...
class InputSanitizationService {
   ...
   */
  public sanitizeText(input: string, maxLength?: number): SanitizationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    const originalLength = input.length;
    ...
    try {
      ...
      errors.push(
        `Text sanitization failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      ...
      fieldRules.forEach((rule) => {
        switch (rule.type) {
          ...
          case "doh2025Compliance":
            if (sanitizedValue) {
              const dohValidation =
                this.validateDOH2025Requirements(sanitizedValue);
          ...
          case "serviceCodePricing":
            if (sanitizedValue && rule.value) {
              const pricingValidation = this.validateServiceCodePricing(
                sanitizedValue,
                rule.value.price,
              );
  ...
  private isValidPhone(phone: string): boolean {
    const phoneRegex =
      /^(\+971|00971|971)?[\s-]?[0-9]{1,2}[\s-]?[0-9]{3}[\s-]?[0-9]{4}$/;
   ...
   */
  private validateDOH2025Requirements(data: any): {
    isValid: boolean;
    errors: string[];
    warnings: string[];
  } {
    ...
    if (
      ...
      if (hoursLate >= 4) {
        errors.push(
          `Critical submission delay: ${hoursLate} hours after 8:00 AM deadline`,
        );
      } else {
        warnings.push(
          `Late submission: ${hoursLate} hours after 8:00 AM deadline`,
        );
      }
    }
   ...
   */
  private sanitizeNewDataFields(data: any): any {
   ...
   */
  private validateServiceCodePricing(
    serviceCode: string,
    price: number,
  ): {
    isValid: boolean;
    ...
    const expectedPrice =
      servicePricing[serviceCode as keyof typeof servicePricing];
   ...
   */
  private isMSCCompliant(data: any): boolean {
    const mscDeadline = new Date("2025-05-14");
    const currentDate = new Date();
   ...
   */
  private isWithinSubmissionDeadline(): boolean {
    const uaeTime = new Date().toLocaleString("en-US", {
      timeZone: "Asia/Dubai",
    });
    ...
    return (
      currentTime <= deadline || currentTime.getDate() !== deadline.getDate()
    );
  }
   ...
   */
  private isWheelchairPreApprovalValid(data: any): boolean {
    ...
    if (data.wheelchairRequest && currentDate >= effectiveDate) {
      ...
      if (
        data.documents?.some((doc: string) => deprecatedForms.includes(doc))
      ) {
        return false;
      }
    }
   ...
   */
  private isFaceToFaceAssessmentValid(data: any): boolean {
...
export const useInputSanitization = () => {
  ...
  const sanitizeHtml = React.useCallback(
    (input: string, options?: SanitizationOptions) => {
      return sanitizer.sanitizeHtml(input, options);
    },
    [sanitizer],
  );

  const sanitizeText = React.useCallback(
