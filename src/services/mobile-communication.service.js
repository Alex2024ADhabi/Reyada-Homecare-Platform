...
class MobileCommunicationService {
    constructor() {
        ...
        Object.defineProperty(this, "isRecognitionActive", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        });
        Object.defineProperty(this, "offlineMessageQueue", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "pushSubscription", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "voiceRecognitionCallbacks", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "encryptionKey", {
            ...
            configurable: true,
            writable: true,
            value: "mobile-comm-2024"
        });
        Object.defineProperty(this, "medicalTerms", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: [
     ...
     */
    async startVoiceRecognition(config) {
     ...
     */
    removeVoiceRecognitionCallback(callback) {
     ...
     */
    queueOfflineMessage(message, priority = "medium") {
    ...
    async processOfflineQueue() {
        ...
        for (const offlineMessage of sortedQueue) {
            try {
                ...
                console.error(`Failed to send offline message ${offlineMessage.id}:`, error);
        ...
        this.offlineMessageQueue = this.offlineMessageQueue.filter((msg) => !processedIds.includes(msg.id) && !failedIds.includes(msg.id));
        this.saveOfflineQueue();
    ...
    getOfflineQueueStatus() {
        const priorityBreakdown = this.offlineMessageQueue.reduce((acc, msg) => {
            acc[msg.priority] = (acc[msg.priority] || 0) + 1;
            return acc;
        }, {});
        const oldestMessage = this.offlineMessageQueue.length > 0
            ? this.offlineMessageQueue.reduce((oldest, msg) => new Date(msg.queuedAt) < new Date(oldest.queuedAt) ? msg : oldest).queuedAt
            : undefined;
        return {
     ...
     */
    async initializeCameraIntegration() {
        try {
            ...
            const capabilities = {
                ...
                flashSupported: videoDevices.some((device) => device.label.toLowerCase().includes("back")),
                zoomSupported: true,
     ...
     */
    async captureWoundImage(options = {}) {
        try {
            const constraints = {
                video: {
            ...
            if (options.annotations) {
                ...
                options.annotations.measurements.forEach((measurement, index) => {
                    ...
                    context.strokeRect(measurement.x, measurement.y, measurement.width, measurement.height);
     ...
     */
    async startEnhancedVoiceRecognition(config) {
            ...
            if (this.speechRecognition) {
                ...
                this.speechRecognition.onresult = async (event) => {
                    await this.processEnhancedSpeechResult(event, {
                        medicalTerminology: medicalTerminologies[config.medicalSpecialty || "general"],
     ...
     */
    async processEnhancedSpeechResult(event, options) {
        const results = event.results;
        const lastResult = results[results.length - 1];
        let transcript = lastResult[0].transcript;
        const confidence = lastResult[0].confidence;
        ...
        transcript = this.applyEnhancedMedicalCorrections(transcript, options.medicalTerminology);
        ...
        const result = {
            ...
            alternatives: Array.from(lastResult)
                .slice(1)
                .map((alt) => ({
                transcript: this.applyEnhancedMedicalCorrections(alt.transcript, options.medicalTerminology),
                confidence: alt.confidence,
            })),
        };
     ...
     */
    applyEnhancedMedicalCorrections(transcript, medicalTerms) {
        let corrected = transcript.toLowerCase();
     ...
     */
    async initializeOfflineVoiceRecognition(specialty) {
        try {
     ...
     */
    async storeOfflineVoiceData(data) {
        try {
            const existingData = JSON.parse(localStorage.getItem("offline_voice_data") || "[]");
     ...
     */
    async initializePushNotifications(config) {
    ...
    saveOfflineQueue() {
        try {
            localStorage.setItem("mobile_comm_offline_queue", JSON.stringify(this.offlineMessageQueue));
        }
        catch (error) {
