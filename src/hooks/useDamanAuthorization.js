...
export function useDamanAuthorization() {
    ...
    const submitAuthorization = useCallback(async (authorizationData) => {
        setIsLoading(true);
        setError(null);
        try {
            ...
            try {
                ...
                if (sanitizedData.requestedDuration) {
                    sanitizedData.requestedDuration = Number(sanitizedData.requestedDuration);
                ...
                throw new Error(`Authorization data validation failed: ${errorMessage}`);
            }
            if (isOnline) {
                // Online mode - submit directly
                const result = await ApiService.submitDamanAuthorization(sanitizedData);
                ...
                try {
                    ...
                    throw new Error(`Failed to create valid queue data: ${errorMessage}`);
                }
                ...
                try {
                    ...
                    if (!parsedOfflineResponse.success ||
                        !parsedOfflineResponse.referenceNumber) {
                        throw new Error("Offline response missing required fields");
                    }
                }
                catch (offlineError) {
                    const errorMessage = offlineError instanceof Error
                        ? offlineError.message
                        : "Offline response validation failed";
                    throw new Error(`Failed to create valid offline response: ${errorMessage}`);
                }
                return offlineResponse;
            }
        }
        catch (err) {
            const errorMessage = err instanceof Error ? err.message : "Failed to submit authorization";
    ...
    const trackAuthorization = useCallback(async (referenceNumber) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
                // Online mode - get status from API
                const result = await ApiService.getDamanAuthorizationStatus(referenceNumber);
                return result;
            }
            else {
                ...
                if (referenceNumber.startsWith("OFFLINE-")) {
                    return {
                        referenceNumber,
                        status: "pending",
                        comments: "This authorization will be processed when you're back online",
            ...
            setError(err.message || `Failed to track authorization ${referenceNumber}`);
            throw err;
        }
        finally {
            setIsLoading(false);
    ...
    const getAuthorizationDetails = useCallback(async (authorizationId) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
                // Online mode - get details from API
                const result = await ApiService.getDamanAuthorizationDetails(authorizationId);
                return result;
            }
            else {
            ...
            console.error(`Failed to get authorization details ${authorizationId}:`, err);
    ...
    const uploadAdditionalDocuments = useCallback(async (authorizationId, files) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
                ...
                const result = await ApiService.uploadAdditionalDocuments(authorizationId, formData);
                return result;
            }
            else {
            ...
            console.error(`Failed to upload additional documents for ${authorizationId}:`, err);
            setError(err.message ||
    ...
    const registerForNotifications = useCallback(async (authorizationId, referenceNumber, notificationPreferences) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
                ...
                const result = await ApiService.registerForAuthorizationWebhooks(authorizationId, webhookData);
                return result;
            }
            else {
            ...
            console.error(`Failed to register for notifications for ${authorizationId}:`, err);
            setError(err.message ||
