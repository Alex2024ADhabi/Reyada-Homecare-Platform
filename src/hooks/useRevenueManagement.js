...
export function useRevenueManagement() {
    ...
    const [accountsReceivableAging, setAccountsReceivableAging] = useState(null);
    ...
    const fetchAccountsReceivableAging = useCallback(async (params) => {
        if (!isOnline) {
    ...
    const fetchDenialAnalytics = useCallback(async (params) => {
        if (!isOnline) {
    ...
    // Record denial
    const recordDenial = useCallback(async (denialData) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
        ...
        try {
            if (isOnline) {
                ...
                if (appealData.denialId) {
                    ...
                    const denials = await offlineService.getDenialManagementsByClaimId(appealData.claimId);
                    const denial = denials.find((d) => d.denialId === appealData.denialId);
                    if (denial && denial.id) {
                        await offlineService.updateDenialManagementStatus(denial.id, "active", "in_progress");
    ...
    const generateRevenueReport = useCallback(async (reportParams) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
    ...
    const getPaymentReconciliation = useCallback(async (params) => {
        if (!isOnline) {
    ...
    const processPaymentReconciliation = useCallback(async (reconciliationData) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
                const result = await ApiService.processPaymentReconciliation(reconciliationData);
                return result;
            }
            else {
                // Offline mode - save locally
    ...
    const getPayerPerformanceAnalytics = useCallback(async (params) => {
        if (!isOnline) {
    ...
    const generateCashFlowProjection = useCallback(async (params) => {
        if (!isOnline) {
    ...
    const getKPIDashboard = useCallback(async (params) => {
        if (!isOnline) {
    ...
    // Submit batch claims
    const submitBatchClaims = useCallback(async (claimsData) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
    ...
    // Update claim status
    const updateClaimStatus = useCallback(async (claimId, status, notes) => {
        setIsLoading(true);
        setError(null);
        try {
            if (isOnline) {
                const result = await ApiService.updateClaimStatus(claimId, status, notes);
                return result;
            }
            else {
