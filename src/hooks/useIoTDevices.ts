...
interface UseIoTDevicesReturn {
  ...
  // Device Management
  addDevice: (device: Omit<IoTDevice, "id">) => Promise<void>;
  updateDevice: (id: string, updates: Partial<IoTDevice>) => Promise<void>;
  removeDevice: (id: string) => Promise<void>;
  executeAction: (action: DeviceManagementAction) => Promise<void>;
  ...
  getMedicationData: (
    deviceId: string,
    timeRange?: { start: Date; end: Date },
  ) => Promise<MedicationDispenserReading[]>;
  getFallDetectionData: (
    deviceId: string,
    timeRange?: { start: Date; end: Date },
  ) => Promise<FallDetectionReading[]>;
...
export function useIoTDevices(): UseIoTDevicesReturn {
    ...
    const interval = setInterval(() => {
      setDevices((prevDevices) =>
        prevDevices.map((device) => ({
          ...
          status:
            Math.random() > 0.05 ? device.status : ("offline" as DeviceStatus),
  ...
  const addDevice = useCallback(async (deviceData: Omit<IoTDevice, "id">) => {
    try {
  ...
  const updateDevice = useCallback(
    async (id: string, updates: Partial<IoTDevice>) => {
      try {
        setDevices((prev) =>
          prev.map((device) =>
  ...
  const executeAction = useCallback(async (action: DeviceManagementAction) => {
    try {
  ...
  const getVitalSigns = useCallback(
    async (
      ...
      const mockReadings: VitalSignsReading[] = Array.from(
        { length: 24 },
        (_, i) => ({
  ...
  const getMedicationData = useCallback(
    async (
      deviceId: string,
      timeRange?: { start: Date; end: Date },
    ): Promise<MedicationDispenserReading[]> => {
      // Mock medication data
      const mockReadings: MedicationDispenserReading[] = Array.from(
        { length: 10 },
        (_, i) => ({
  ...
  const getFallDetectionData = useCallback(
    async (
      deviceId: string,
      timeRange?: { start: Date; end: Date },
    ): Promise<FallDetectionReading[]> => {
      // Mock fall detection data
      const mockReadings: FallDetectionReading[] = Array.from(
        { length: 5 },
        (_, i) => ({
  ...
  const getEnvironmentalData = useCallback(
    async (
      ...
      const mockReadings: EnvironmentalReading[] = Array.from(
        { length: 144 },
        (_, i) => ({
