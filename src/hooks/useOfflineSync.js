...
export function useOfflineSync() {
    ...
    const syncPendingData = useCallback(async () => {
        ...
        try {
            ...
            for (const priority of ["critical", "high", "medium", "low"]) {
                const itemTypes = syncPriorities[priority];
                for (const itemType of itemTypes) {
                    const items = pendingData[itemType];
                    ...
                    for (const item of items) {
                        try {
                            ...
                            if (conflictResolution.hasConflict) {
                                console.warn(`Conflict detected for ${itemType} ${item.id}:`, conflictResolution.conflicts);
                            ...
                            if (itemType === "clinicalForms" ||
                                ...
                                await offlineService.updatePaymentReconciliationStatus(item.id, "synced");
                            }
                            else if (itemType === "denialManagements") {
                                await offlineService.updateDenialManagementStatus(item.id, "synced");
                            }
                            ...
                            if (item.retryCount >= 3) {
                                console.error(`Max retries exceeded for ${itemType} ${item.id}`);
                            }
                        }
                    }
                }
    ...
    const resolveDataConflicts = async (item) => {
        try {
    ...
    const checkServerVersion = async (itemId) => {
        try {
    ...
    const mergeDataChanges = (localData, serverData) => {
        try {
    ...
    const syncWithRetry = async (item, itemType, maxRetries = 3) => {
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                console.warn(`Sync attempt ${attempt} failed for ${itemType} ${item.id}, retrying...`);
            }
        }
    };
    ...
    const saveFormData = useCallback(async (formType, formData) => {
        try {
