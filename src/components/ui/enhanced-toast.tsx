...
const variantStyles = {
  ...
  "authorization-pending":
    "bg-purple-50 border-purple-300 text-purple-900 shadow-purple-100",
  "security-threat":
    "bg-red-100 border-red-400 text-red-950 shadow-red-200 ring-2 ring-red-300",
  "security-warning":
    "bg-orange-50 border-orange-300 text-orange-900 shadow-orange-100",
  "security-success":
    "bg-green-100 border-green-300 text-green-900 shadow-green-100",
  "vulnerability-critical":
    "bg-red-200 border-red-500 text-red-950 shadow-red-300 ring-2 ring-red-400 animate-pulse",
  "vulnerability-high": "bg-red-100 border-red-400 text-red-900 shadow-red-200",
  "intrusion-detected":
    "bg-red-200 border-red-600 text-red-950 shadow-red-300 ring-4 ring-red-500 animate-pulse",
  "malware-detected":
    "bg-red-150 border-red-500 text-red-950 shadow-red-250 ring-3 ring-red-400",
  "data-breach":
    "bg-red-200 border-red-700 text-red-950 shadow-red-400 ring-4 ring-red-600 animate-pulse",
  "penetration-test":
...
export const EnhancedToast: React.FC<EnhancedToastProps> = ({
  ...
  const Icon =
    loading || variant === "authorization-pending"
      ? Loader2
      : variantIcons[variant];
  const isComplianceVariant =
    variant.startsWith("compliance-") ||
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden flex items-start gap-3 p-4 rounded-lg border shadow-lg max-w-md transition-all duration-300 ease-in-out",
        variantStyles[variant],
        isVisible && !isExiting
          ? "translate-x-0 opacity-100 animate-in slide-in-from-right-full"
          : "translate-x-full opacity-0",
        isExiting && "translate-x-full opacity-0",
        complianceLevel && complianceLevelColor[complianceLevel],
        isComplianceVariant && "ring-2 ring-offset-2",
        className,
      )}
      role="alert"
      aria-live={isComplianceVariant ? "assertive" : "polite"}
      aria-label={accessibilityLabel || `${variant} notification: ${title}`}
    ...
    >
      ...
      {shouldShowProgress && (
        <div className="absolute bottom-0 left-0 h-1 bg-gray-200 w-full">
          <div
            ...
            role="progressbar"
            aria-valuenow={progress !== undefined ? progress : progressState}
            aria-valuemin={0}
            aria-valuemax={100}
          />
        </div>
        ...
        <div className="absolute top-2 right-12 flex items-center gap-1">
          <div
            className={cn(
              ...
              complianceLevel === "critical" && "bg-red-600 animate-pulse",
      ...
      <Icon
        className={cn(
          ...
          loading ? "animate-spin text-blue-600" : iconColors[variant],
        )}
      />
