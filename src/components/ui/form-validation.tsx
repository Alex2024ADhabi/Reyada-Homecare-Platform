...
export const ValidationSuccess: React.FC<ValidationSuccessProps> = ({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-1 text-sm text-green-600 mt-1",
        className,
      )}
    >
...
// Validation info/help text
interface ValidationInfoProps {
  message: string;
  className?: string;
}

export const ValidationInfo: React.FC<ValidationInfoProps> = ({
  message,
  className,
}) => {
  return (
    <div
      className={cn(
        "flex items-center gap-1 text-sm text-blue-600 mt-1",
        className,
      )}
    >
...
export const ValidatedField: React.FC<ValidatedFieldProps> = ({
  ...
  return (
    <div className={cn("space-y-1", className)}>
      ...
      <ValidationSuccess show={success && !error} />
      {info && !error && <ValidationInfo message={info} />}
    </div>
  );
};

// Real-time validation hook with enhanced JSON safety and DOH compliance
export const useRealTimeValidation = <T extends z.ZodSchema>(
  schema: T,
  value: any,
  field: string,
  dohCompliance = false,
  ...
  >(null);
  const [dohRequirements, setDohRequirements] = React.useState<string[]>([]);
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (dohCompliance) {
        const dohValidationResult = validateDOHCompliance(
          field,
          sanitizedValue,
        );
      ...
      if (fieldSchema && typeof fieldSchema.parse === "function") {
        ...
        setIsValid(true);
        setError(null);
        if (!dohCompliance) setComplianceLevel("passed");
      } else {
...
const validateDOHCompliance = (field: string, value: any) => {
  const dohRules = {
    emiratesId: {
  ...
  const rule = dohRules[field as keyof typeof dohRules];
  if (!rule) {
    return { level: "low" as const, requirements: [], error: null };
  }
...
export const DOH2025ValidationSchema = z.object({
  ...
  emiratesId: z
    ...
    .regex(
      /^\d{3}-\d{4}-\d{7}-\d{1}$/,
      "Invalid Emirates ID format per DOH standards",
    ),
...
export const EnhancedClinicalFormValidationSchema =
  ClinicalFormValidationSchema.extend({
...
export const ValidationUtils = {
  ...
  validateInsurancePolicy: (policy: string): boolean => {
  ...
  validateBloodPressure: (bp: string): boolean => {
    ...
    return (
      systolic >= 70 && systolic <= 250 && diastolic >= 40 && diastolic <= 150
    );
  },
  ...
  validateHeartRate: (hr: number): boolean => {
    if (typeof hr !== "number" || isNaN(hr)) return false;
    return hr >= 30 && hr <= 200;
  },

  // Oxygen saturation validation
  validateOxygenSaturation: (spo2: number): boolean => {
    if (typeof spo2 !== "number" || isNaN(spo2)) return false;
    return spo2 >= 70 && spo2 <= 100;
  },

  // DOH 2025 specific validations
  validateDOHAuthorization: (authNumber: string): boolean => {
    if (!authNumber || typeof authNumber !== "string") return false;
    const regex = /^AUTH\d{8}$/;
    return regex.test(authNumber);
  },

  validateServiceCode: (code: string): boolean => {
    if (!code || typeof code !== "string") return false;
    const regex = /^[A-Z]{2}\d{3}$/;
    return regex.test(code);
  },

  validateComplianceLevel: (level: string): boolean => {
    const validLevels = ["critical", "high", "medium", "low", "passed"];
    return validLevels.includes(level);
  },

  // DOH Patient Safety Taxonomy Validations (CN_19_2025)
  validatePatientSafetyTaxonomy: (
    taxonomy: any,
  ): {
    isValid: boolean;
    errors: string[];
    ...
    requiredLevels.forEach((level) => {
      if (taxonomy[level] && taxonomy[level].trim().length > 0) {
        ...
        errors.push(
          `${level.replace("_", " ")} is required for DOH compliance`,
        );
      }
    });
    ...
    if (
      ...
      errors.push(
        "Taxonomy version must be CN_19_2025 for current DOH compliance",
      );
    }
  ...
  validatePatientSafetyImpact: (
    ...
    if (
      typeof impact.preventability_score !== "number" ||
  ...
  validateIncidentClassificationCompleteness: (
    incident: any,
  ): {
    completeness: number;
    ...
    if (incident.doh_taxonomy) {
      const taxonomyValidation = ValidationUtils.validatePatientSafetyTaxonomy(
        incident.doh_taxonomy,
      );
      ...
      missingElements.push(
        "DOH Patient Safety Taxonomy classification missing",
      );
    }

    // Check patient safety impact (30% weight)
    if (incident.patient_safety_impact) {
      const impactValidation = ValidationUtils.validatePatientSafetyImpact(
        incident.patient_safety_impact,
      );
  ...
  validateDOHTimestamp: (timestamp: string): boolean => {
    try {
  ...
  checkDOHCompliance: (
    ...
    // Determine compliance level
    let level: "critical" | "high" | "medium" | "low" | "passed";
    if (violations.length === 0) {
      level = "passed";
    } else if (
      violations.some(
        (v) => v.includes("Emirates ID") || v.includes("authorization"),
      )
    ) {
      level = "critical";
