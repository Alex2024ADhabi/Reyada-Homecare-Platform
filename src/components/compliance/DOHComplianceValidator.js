    ...
    validationFunction: async (data) => {
      ...
      if (incompleteDomains.length > 0) {
        issues.push({
          ...
          severity: "critical",
          message: `, $
                        }, { incompleteDomains, : .length }, domains, incomplete, $, { incompleteDomains, : .join(', ') } `,
      ...
      if (data.staffCredentials?.licenseExpiryDate) {
        ...
        if (daysToExpiry <= 0) {
          ...
          issues.push({
            ...
            severity: "high",
            message: `, Staff, license, expires in $, { daysToExpiry }, days, ",, regulation, "DOH Professional Licensing Standards", field, "staffCredentials.licenseExpiryDate", correctionRequired, true);
                    }
                    ;
                    score -= 20;
                }
      ...
      if (enableNineDomainsValidation) {
        for (const rule of DOH_NINE_DOMAINS_RULES) {
          try {
            ...
            console.error(`, DOH, 9: -Domain, rule, $
                        }, { rule, : .id }, failed, `, error);
            results.push({
              passed: false,
              score: 0,
              issues: [{
...
$;
{
    validationResult.passed ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
}
`}>
                  {validationResult.passed ? 'COMPLIANT' : 'NON-COMPLIANT'}
                </Badge>
