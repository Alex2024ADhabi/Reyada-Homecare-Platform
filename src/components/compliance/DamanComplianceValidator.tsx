...
interface DamanComplianceRule {
  id: string;
  name: string;
  description: string;
  category: string;
  severity: "critical" | "high" | "medium" | "low";
  validationFunction: (data: any) => Promise<DamanComplianceResult>;
}

interface DamanComplianceResult {
  passed: boolean;
  score: number;
  issues: DamanComplianceIssue[];
  recommendations: string[];
}

interface DamanComplianceIssue {
  id: string;
  severity: "critical" | "high" | "medium" | "low";
  message: string;
  policy: string;
  field?: string;
  impactsReimbursement: boolean;
}

interface DamanComplianceValidatorProps {
  data?: any;
  onValidationComplete?: (result: DamanComplianceResult) => void;
  className?: string;
}

const DAMAN_COMPLIANCE_RULES: DamanComplianceRule[] = [
  {
    id: "daman-001",
    name: "MSC Plan Extension Compliance",
    description:
      "Validates MSC plan extension requirements and 90-day duration limits with enhanced validation",
    category: "Authorization",
    severity: "critical",
    validationFunction: async (data) => {
      const issues: DamanComplianceIssue[] = [];
      ...
      if (data.mscPlanExtension && currentDate > mscDeadline) {
        issues.push({
          ...
          severity: "critical",
          message:
            "MSC plan extension has expired (deadline: May 14, 2025). No further extensions available.",
          policy:
            "Defense Health Organization (MSC) Plan Extension - Final Deadline",
      ...
      if (
        ...
        issues.push({
          ...
          severity: "high",
          message:
            "MSC plan extension is in final grace period. Deadline: May 14, 2025",
      ...
      if (data.mscPlanExtension && data.initialVisitDate) {
        ...
        const daysDiff = Math.floor(
          (initialVisit.getTime() - atcEffectiveDate.getTime()) /
            (1000 * 60 * 60 * 24),
        );
      ...
      if (score < 100) {
        recommendations.push(
          "Ensure MSC plan extensions are submitted before May 14, 2025 deadline",
        );
        recommendations.push("Limit MSC extension duration to maximum 90 days");
        recommendations.push(
          "Schedule initial visit within 90 days of ATC effective date",
        );
        recommendations.push(
          "Update payment terms to 30 days for CN_2025 compliance",
        );
      }
  ...
  },
  {
    ...
    description:
      "Validates new homecare service codes (17-25-x) and deprecated codes removal",
    ...
    validationFunction: async (data) => {
      ...
      const usedDeprecatedCodes = serviceCodes.filter((code: string) =>
      ...
      if (usedDeprecatedCodes.length > 0) {
        ...
        const detailedMessage = usedDeprecatedCodes
          .map(
            (code) =>
              `${code}: ${deprecatedCodeDetails[code as keyof typeof deprecatedCodeDetails] || "Unknown deprecated code"}`,
          )
          .join("; ");
      ...
      const validCodesUsed = serviceCodes.filter((code: string) =>
      ...
      if (serviceCodes.length > 0 && validCodesUsed.length === 0) {
        issues.push({
          ...
          severity: "critical",
          message:
            "No valid service codes (17-25-1 to 17-25-5) found in submission",
      ...
      serviceCodes.forEach((code: string) => {
        if (
          ...
          const expectedPrice =
            servicePricing[code as keyof typeof servicePricing];
      ...
      if (score < 100) {
        recommendations.push(
          "Remove all deprecated service codes (17-26-1 to 17-26-4) from submissions",
        );
        recommendations.push(
          "Use only valid service codes (17-25-1 to 17-25-5) for homecare services",
        );
        recommendations.push(
          "Verify service code pricing matches Daman approved rates",
        );
        recommendations.push(
          "Update service selection dropdowns to reflect new codes only",
        );
      }
  ...
  },
  {
    ...
    description:
      "Validates 8:00 AM daily submission deadline and grace periods",
    category: "Timeline",
    severity: "high",
    validationFunction: async (data) => {
      const issues: DamanComplianceIssue[] = [];
      ...
      if (
        currentUAETime > submissionDeadlineUAE &&
        currentUAETime.getDate() === submissionDeadlineUAE.getDate()
      ) {
        const hoursLate = Math.floor(
          (currentUAETime.getTime() - submissionDeadlineUAE.getTime()) /
            (1000 * 60 * 60),
        );
      ...
      if (data.mscClaim && currentTime > mscGracePeriod) {
        issues.push({
          ...
          severity: "critical",
          message:
            "MSC claims grace period expired (March 1, 2025). Final reconciliation deadline passed",
      ...
      if (data.homecareService && data.lastSubmissionDate) {
        ...
        if (daysSinceLastSubmission > 30) {
          issues.push({
            ...
            severity: "medium",
            message:
              "Homecare services must be submitted monthly (30-day service period)",
      ...
      if (score < 100) {
        recommendations.push(
          "Submit claims before 8:00 AM daily deadline to avoid escalation",
        );
        recommendations.push(
          "Complete MSC final reconciliation before March 1, 2025",
        );
        recommendations.push(
          "Ensure homecare invoices are submitted monthly (30-day periods)",
        );
        recommendations.push(
  ...
  },
  {
    ...
    description:
      "Validates wheelchair pre-approval forms and homecare allocation documents",
    ...
    validationFunction: async (data) => {
      ...
      if (data.wheelchairRequest) {
        ...
        if (
          ...
          if (!data.documents?.includes("Wheelchair Pre-approval Form")) {
            issues.push({
              ...
              severity: "high",
              message:
                "Wheelchair pre-approval form will be mandatory from May 1, 2025. PT form will no longer be accepted. Please update your documentation.",
        ...
        if (currentDate >= wheelchairFormDeadline) {
          if (!data.documents?.includes("Wheelchair Pre-approval Form")) {
            issues.push({
              ...
              severity: "critical",
              message:
                "Wheelchair pre-approval form is mandatory (effective May 1, 2025). PT form no longer accepted. Claims will be rejected without proper documentation.",
          ...
          if (
            ...
            issues.push({
              ...
              severity: "critical",
              message:
                "PT/Physiotherapy forms are no longer accepted for wheelchair requests (effective May 1, 2025). Use Wheelchair Pre-approval Form only.",
        ...
        if (data.wheelchairFormDate) {
          ...
          const daysSinceForm = Math.floor(
            (currentDate.getTime() - formDate.getTime()) /
              (1000 * 60 * 60 * 24),
          );
          ...
          if (daysSinceForm > 30) {
            issues.push({
              ...
              severity: "high",
              message:
                "Wheelchair pre-approval form is older than 1 month and must be renewed",
        ...
        const missingWheelchairDocs = requiredWheelchairDocs.filter(
      ...
      if (data.homecareAllocation) {
        ...
        if (
          currentDate >= homecarePreparationDate &&
          ...
          if (!data.documents?.includes("Face-to-Face Assessment (OpenJet)")) {
            issues.push({
              ...
              severity: "medium",
              message:
                "Face-to-face assessment through OpenJet will be mandatory from Feb 24, 2025. Please prepare documentation.",
        ...
        if (currentDate >= homecareAutomationDate) {
          if (!data.documents?.includes("Face-to-Face Assessment (OpenJet)")) {
            issues.push({
              ...
              severity: "critical",
              message:
                "Face-to-face assessment form completion through OpenJet is mandatory for homecare allocation (effective Feb 24, 2025). Manual assessments no longer accepted.",
          ...
          if (!data.openJetIntegration) {
            issues.push({
              ...
              severity: "critical",
              message:
                "OpenJet system integration is required for automated homecare allocation (effective Feb 24, 2025)",
        ...
        const missingHomecareDocs = requiredHomecareDocs.filter(
          (doc) => !data.documents?.includes(doc),
        );
        ...
        if (data.medicalReportDate) {
          ...
          if (daysSinceReport > 90) {
            issues.push({
              ...
              severity: "medium",
              message:
                "Medical report is older than 3 months and should be updated",
      ...
      if (score < 100) {
        recommendations.push(
          "Use new wheelchair pre-approval form (effective May 1, 2025) - PT form no longer accepted",
        );
        recommendations.push(
          "Complete face-to-face assessment through OpenJet for homecare allocation",
        );
        recommendations.push(
          "Ensure all required documents are attached before submission",
        );
        recommendations.push(
          "Keep medical reports updated (not older than 3 months)",
        );
        recommendations.push(
          "Maintain wheelchair form validity within 1-month period",
        );
      }
    ...
    validationFunction: async (data) => {
      ...
      if (!data.openJetIntegration) {
        issues.push({
          ...
          severity: "medium",
          message:
            "OpenJet integration required for provider services and queries",
      ...
      if (data.mscCommunication && !data.mscDesignatedChannel) {
        issues.push({
          ...
          severity: "medium",
          message:
            "MSC communications must use designated channel (MSC@damanhealth.ae)",
      ...
      if (
        ...
        issues.push({
          ...
          severity: "medium",
          message:
            "ABM enrollment queries must use OpenJet channel (effective March 21, 2025)",
      ...
      if (score < 100) {
        recommendations.push(
          "Use official UAE-hosted email domains for all communications",
        );
        recommendations.push(
          "Integrate with OpenJet system for provider services and queries",
        );
        recommendations.push(
          "Use designated MSC communication channel for MSC-related matters",
        );
        recommendations.push(
          "Utilize OpenJet for ABM enrollment queries and escalations",
        );
      }
...
export default function DamanComplianceValidator({
  data = {},
  onValidationComplete,
  className,
}: DamanComplianceValidatorProps) {
  const [isValidating, setIsValidating] = useState(false);
  const [validationResult, setValidationResult] =
    useState<DamanComplianceResult | null>(null);
  ...
  const runDamanValidation = useCallback(async () => {
    ...
    try {
      const results: DamanComplianceResult[] = [];
      ...
      const sanitizedData = {
        ...
        mscPlanExtension:
          data && typeof data === "object" && data.mscPlanExtension
            ? Boolean(data.mscPlanExtension)
            : false,
        requestedDuration:
          data && typeof data === "object" && data.requestedDuration
            ? typeof data.requestedDuration === "number"
              ? data.requestedDuration
              : Number(data.requestedDuration) || 0
            : 0,
        initialVisitDate:
          data && typeof data === "object" && data.initialVisitDate
            ? String(data.initialVisitDate)
            : null,
        atcEffectiveDate:
          data && typeof data === "object" && data.atcEffectiveDate
            ? String(data.atcEffectiveDate)
            : null,
        paymentTerms:
          data && typeof data === "object" && data.paymentTerms
            ? String(data.paymentTerms)
            : "30_days",
        serviceCodes:
          data && typeof data === "object" && Array.isArray(data.serviceCodes)
            ? data.serviceCodes.filter(
        ...
        servicePricing:
          ...
          data.servicePricing !== null
            ...
            : {},
        mscClaim:
          data && typeof data === "object" && data.mscClaim
            ? Boolean(data.mscClaim)
            : false,
        serviceDate:
          data && typeof data === "object" && data.serviceDate
            ? String(data.serviceDate)
            : null,
        homecareService:
          data && typeof data === "object" && data.homecareService
            ? Boolean(data.homecareService)
            : false,
        lastSubmissionDate:
          data && typeof data === "object" && data.lastSubmissionDate
            ? String(data.lastSubmissionDate)
            : null,
        wheelchairRequest:
          data && typeof data === "object" && data.wheelchairRequest
            ? Boolean(data.wheelchairRequest)
            : false,
        documents:
          data && typeof data === "object" && Array.isArray(data.documents)
            ? data.documents.filter((doc) => doc && typeof doc === "string")
            : [],
        wheelchairFormDate:
          data && typeof data === "object" && data.wheelchairFormDate
            ? String(data.wheelchairFormDate)
            : null,
        medicalReportDate:
          data && typeof data === "object" && data.medicalReportDate
            ? String(data.medicalReportDate)
            : null,
        homecareAllocation:
          data && typeof data === "object" && data.homecareAllocation
            ? Boolean(data.homecareAllocation)
            : false,
        providerEmail:
          data && typeof data === "object" && data.providerEmail
            ? String(data.providerEmail)
            : "",
        openJetIntegration:
          data && typeof data === "object" && data.openJetIntegration
            ? Boolean(data.openJetIntegration)
            : false,
        mscCommunication:
          data && typeof data === "object" && data.mscCommunication
            ? Boolean(data.mscCommunication)
            : false,
        mscDesignatedChannel:
          data && typeof data === "object" && data.mscDesignatedChannel
            ? Boolean(data.mscDesignatedChannel)
            : false,
        abmEnrollment:
          data && typeof data === "object" && data.abmEnrollment
            ? Boolean(data.abmEnrollment)
            : false,
        abmOpenJetChannel:
          data && typeof data === "object" && data.abmOpenJetChannel
      ...
      for (const rule of DAMAN_COMPLIANCE_RULES) {
        try {
          ...
          const validatedResult: DamanComplianceResult = {
            ...
            issues: Array.isArray(result?.issues)
              ? result.issues.map((issue) => ({
                  ...
                  impactsReimbursement: Boolean(
                    issue?.impactsReimbursement ?? false,
                  ),
                }))
              : [],
      ...
      const allIssues: DamanComplianceIssue[] = [];
      ...
      const reimbursementImpactingIssues = allIssues.filter(
        (issue) => issue.impactsReimbursement,
      );
      const overallPassed = criticalIssues.length === 0;

      // Create final result with comprehensive validation
      const finalResult: DamanComplianceResult = {
      ...
      try {
        if (overallPassed) {
          if (typeof toast === "function") {
            ...
            console.log(
              `✅ Daman Compliance Validated - Score: ${averageScore.toFixed(1)}%`,
            );
          }
        } else {
          if (typeof toast === "function") {
            ...
            console.warn(
              `⚠️ Daman Compliance Issues - ${reimbursementImpactingIssues.length} issues may impact reimbursement`,
            );
          }
        }
      } catch (toastError) {
        ...
        try {
          if (overallPassed) {
            ...
            alert(
              `Daman Compliance Issues - ${reimbursementImpactingIssues.length} issues may impact reimbursement`,
            );
          }
        } catch (alertError) {
      ...
      try {
        if (typeof toast === "function") {
          ...
          alert(
            "Failed to complete Daman compliance validation. Please try again.",
          );
        }
      } catch (toastError) {
        ...
        try {
          alert(
            "Validation error occurred. Please check the console for details.",
          );
        } catch (alertError) {
          console.error("Alert fallback failed:", alertError);
        }
  ...
  return (
    <div className={`bg-white space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center">
            ...
            Daman Compliance Validator
          </h2>
          <p className="text-gray-600 mt-1">
            Comprehensive Daman insurance compliance validation and
            reimbursement optimization
          </p>
        </div>
        <div className="flex items-center space-x-2">
          {validationResult && (
            <Badge className={getScoreBadgeColor(validationResult.score)}>
              {validationResult.passed ? "COMPLIANT" : "NON-COMPLIANT"} (
              {validationResult.score}%)
            </Badge>
          )}
          <Button
            onClick={runDamanValidation}
          ...
          >
            ...
            {isValidating ? "Validating..." : "Validate Daman Compliance"}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {DAMAN_COMPLIANCE_RULES.map((rule) => {
          const icons = {
          ...
          const Icon = icons[rule.category as keyof typeof icons] || Activity;
        ...
        <div className="space-y-4">
          ...
          <Card className="border-l-4 border-l-blue-500">
            <CardHeader>
              ...
              <CardDescription>
                Last validated: {lastValidation?.toLocaleString()} | Issues
                Found: {validationResult.issues.length} | Reimbursement Impact:{" "}
                {
                  validationResult.issues.filter((i) => i.impactsReimbursement)
                    .length
                }{" "}
                issues
              </CardDescription>
                  ...
                  {validationResult.issues
                    ...
                    .map((issue, index) => (
                      <li key={index} className="text-sm">
                        ...
                        <br />
                        <span className="text-xs">Policy: {issue.policy}</span>
                      </li>
                    ))}
                </ul>
              </AlertDescription>
          ...
          {validationResult.issues.filter((i) => i.severity === "high").length >
            ...
            <Alert className="bg-orange-50 border-orange-200">
              ...
              <AlertTitle className="text-orange-800">
                High Priority Issues (
                {
                  validationResult.issues.filter((i) => i.severity === "high")
                    .length
                }
                )
              </AlertTitle>
          ...
          {validationResult.issues.filter((i) => i.severity === "medium")
            .length > 0 && (
          ...
          {validationResult.issues.length === 0 && (
            <Alert className="bg-green-50 border-green-200">
              ...
              <AlertTitle className="text-green-800">
                Full Daman Compliance Achieved
              </AlertTitle>
              <AlertDescription className="text-green-700">
                All Daman insurance requirements have been met. Claim is ready
                for submission with optimal reimbursement potential.
              </AlertDescription>
            </Alert>
          )}
