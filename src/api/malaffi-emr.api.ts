  ...
  try {
    ...
    if (!hasValidCriteria) {
      return res.status(400).json({
        ...
        message:
          "At least one of: Emirates ID, MRN, full name, or phone number is required",
      });
    }
...
router.get("/patients/:patientId/medical-records", async (req, res) => {
  try {
    ...
    const categorizedRecords = {
      consultations: medicalRecords.filter(
  ...
  try {
    ...
    if (!localPatients || !localRecords) {
      return res.status(400).json({
        ...
        message:
          "Both localPatients and localRecords are required for synchronization",
      });
    }
