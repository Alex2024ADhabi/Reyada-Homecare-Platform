...
import { performRealTimeComplianceCheck, getAdministrativeOverview, getStaffCategoryPerformance, createProcessOrchestration, updateProcessOrchestration, getProcessOrchestrationStatus, createMasterPatientIndex, createMasterStaffIndex, createSystemIntegration, logEventProcessing, } from "../administrative-integration.api";
import { monitorIntegrationHealth, optimizeIntegrationPerformance, getIntegrationAnalytics, } from "../integration-intelligence.api";
...
router.get("/overview", async (req, res) => {
    try {
        ...
        const overview = await getAdministrativeOverview(dateFrom, dateTo);
        res.json(overview);
    }
    catch (error) {
...
router.get("/staff-performance", async (req, res) => {
    try {
        const { dateFrom, dateTo } = req.query;
        const performance = await getStaffCategoryPerformance(dateFrom, dateTo);
...
router.put("/workflow/orchestration/:processInstanceId", async (req, res) => {
    try {
        ...
        const process = await updateProcessOrchestration(processInstanceId, updates);
        res.json(process);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.get("/business-continuity/documentation", async (req, res) => {
    try {
        const { backupRecoveryService } = await import("../../services/backup-recovery.service");
        await backupRecoveryService.initialize();
        const documentation = backupRecoveryService.generateBusinessContinuityDocumentation();
...
router.get("/integration-intelligence/systems/:systemId/health", async (req, res) => {
    try {
        const { systemId } = req.params;
        const healthReport = await monitorIntegrationHealth();
        // Filter for specific system
        const systemHealth = healthReport.individualSystemHealth.find((system) => system.systemId === systemId);
        if (!systemHealth) {
        ...
        res.json({
            systemId,
            systemHealth,
            relatedAlerts: healthReport.alertsAndNotifications.filter((alert) => alert.systemId === systemId),
            relatedRecommendations: healthReport.recommendedActions.filter((action) => action.description.toLowerCase().includes(systemId.toLowerCase())),
        });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post("/integration-intelligence/systems/:systemId/optimize", async (req, res) => {
    try {
        const { systemId } = req.params;
        const performanceData = req.body;
        const optimizationPlan = await optimizeIntegrationPerformance(systemId, performanceData);
