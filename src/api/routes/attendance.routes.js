...
import { getAttendanceRecords, getAttendanceById, createAttendanceRecord, updateAttendanceRecord, deleteAttendanceRecord, clockIn, clockOut, getTimesheetSummaries, generateTimesheetSummary, approveTimesheet, getAttendanceAnalytics, sendLateArrivalNotification, sendDOHNotification, escalateLateArrival, flagAttendanceForReview, createStaffCategoryTimesheet, getStaffCategoryTimesheets, createEtihadVaccinationTimesheet, createICONTimesheet, createRHHCSAdminTimesheet, createRHHCSNurseTimesheet, createRHHCSSMOTimesheet, createRHHCSTherapistDriverTimesheet, getAttendanceComplianceReport, } from "../attendance.api";
...
router.post("/timesheets/generate", async (req, res) => {
    try {
        ...
        const summary = await generateTimesheetSummary(employee_id, pay_period_start, pay_period_end);
        res.json(summary);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post("/escalations/late-arrival", async (req, res) => {
    try {
        const { employee_id, late_minutes, employee_info, supervisor_id } = req.body;
        await escalateLateArrival(employee_id, late_minutes, employee_info, supervisor_id);
