...
router.put("/devices/:deviceId/status", async (req, res) => {
  try {
    await edgeComputingService.updateDeviceStatus(
      req.params.deviceId,
      req.body.status,
    );
...
router.post("/devices/:deviceId/optimize", async (req, res) => {
  try {
    const success = await edgeComputingService.optimizeDevice(
      req.params.deviceId,
    );
    res.json({ success });
  } catch (error) {
...
router.get("/workloads/:workloadId", async (req, res) => {
  try {
    const workload = await edgeComputingService.getWorkload(
...
router.post("/conflicts/:conflictId/resolve", async (req, res) => {
  try {
    const success = await edgeComputingService.resolveConflict(
...
router.delete("/cache/:pattern", async (req, res) => {
  try {
    const invalidatedCount = await edgeComputingService.invalidateCache(
      req.params.pattern,
    );
...
router.get("/dashboard", async (req, res) => {
  try {
    const dashboardData =
      await edgeComputingService.getEdgeComputingDashboard();
