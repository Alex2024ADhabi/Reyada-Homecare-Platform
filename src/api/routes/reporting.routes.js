...
import { getReportTemplates, createReportTemplate, updateReportTemplate, deleteReportTemplate, getGeneratedReports, generateReport, approveReport, distributeReport, getReportSchedules, createReportSchedule, updateReportSchedule, deleteReportSchedule, getReportingAnalytics, scheduleEndOfDayReportPreparation, processEndOfDayReportDeadlines, getOverdueReports, getPendingApprovals, } from "../reporting.api";
import { createDataMart, optimizeETLProcesses, monitorDataQuality, performAutomatedValidation, createCustomReport, createSelfServiceWorkspace, scheduleAutomatedReport, exportData, } from "../integration-intelligence.api";
...
router.post("/generate-bi-report", async (req, res) => {
    try {
        ...
        setTimeout(() => {
            console.log(`Business Intelligence report ${reportId} generated successfully`);
        }, 2000);
        res.json({
            success: true,
            reportId,
...
router.get("/compliance-monitoring/dashboard", async (req, res) => {
    try {
        const { getComplianceMonitoringDashboard } = await import("../reporting.api");
...
router.post("/compliance-audits/:auditId/preparation-tasks", async (req, res) => {
    try {
        const { generateComplianceAuditPreparationTasks } = await import("../reporting.api");
        const tasks = await generateComplianceAuditPreparationTasks(req.params.auditId);
...
router.get("/compliance-dashboard/comprehensive", async (req, res) => {
    try {
        const { getComplianceMonitoringDashboard, getUpcomingComplianceAudits, getComplianceAuditCalendar, } = await import("../reporting.api");
        const [monitoringDashboard, upcomingAudits, auditCalendar] = await Promise.all([
        ...
        const comprehensiveDashboard = {
            overview: {
                ...
                active_monitoring_rules: monitoringDashboard.overview.total_active_rules,
            ...
            real_time_monitoring: {
                ...
                critical_issues: monitoringDashboard.real_time_alerts.filter((a) => a.severity === "critical").length,
                automated_actions_today: monitoringDashboard.automated_actions.total_actions_today,
            ...
            compliance_trends: {
                ...
                category_scores: monitoringDashboard.compliance_trends.category_performance,
                trend_direction: "improving",
                forecast_30_days: Math.min(100, monitoringDashboard.overview.overall_compliance_score + 2.5),
            },
            audit_management: {
                ...
                next_audit: upcomingAudits[0]
                    ? {
                        ...
                        days_remaining: Math.ceil((new Date(upcomingAudits[0].schedule_config.start_date).getTime() -
                            new Date().getTime()) /
                            (1000 * 60 * 60 * 24)),
                    }
                    : null,
            ...
            automated_remediation: {
                ...
                successful_remediations: monitoringDashboard.automated_actions.successful_remediations,
                pending_escalations: monitoringDashboard.automated_actions.pending_escalations,
                remediation_success_rate: monitoringDashboard.automated_actions.successful_remediations > 0
            ...
            regulatory_compliance: {
                doh_compliance: monitoringDashboard.compliance_trends.category_performance.doh ||
                    95.2,
                jawda_compliance: monitoringDashboard.compliance_trends.category_performance.jawda ||
                    92.8,
                daman_compliance: monitoringDashboard.compliance_trends.category_performance.daman ||
                    88.5,
                overall_regulatory_score: ((monitoringDashboard.compliance_trends.category_performance.doh ||
                    95.2) +
            ...
            action_items: {
                ...
                this_week: upcomingAudits
                    .filter((a) => {
                    const daysUntil = Math.ceil((new Date(a.schedule_config.start_date).getTime() -
                        new Date().getTime()) /
                        (1000 * 60 * 60 * 24));
                    return daysUntil <= 7;
                })
                ...
                this_month: upcomingAudits
                    ...
                    due_date: new Date(new Date(a.schedule_config.start_date).getTime() -
                        a.schedule_config.preparation_days * 24 * 60 * 60 * 1000).toISOString(),
                    assigned_to: a.audit_team.lead_auditor,
                })),
            },
        };
