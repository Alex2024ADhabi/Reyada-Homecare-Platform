...
import { monitorIntegrationHealth, optimizeIntegrationPerformance, getIntegrationAnalytics, initializeIntegrationIntelligenceData, } from "../integration-intelligence.api";
...
router.get("/performance/:systemId", async (req, res) => {
    try {
        ...
        res.json({
            systemId,
            performanceMetrics: analytics.performanceData.filter((p) => p.system_type === systemId),
            dataFlowMetrics: analytics.dataFlowData.filter((d) => d.source_system === systemId || d.target_system === systemId),
        });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.get("/opportunities", async (req, res) => {
    try {
        ...
        res.json({
            ...
            opportunities: opportunities.sort((a, b) => b.estimatedROI - a.estimatedROI),
        });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.get("/predictions", async (req, res) => {
    try {
        ...
        if (timeHorizon) {
            ...
            predictions = predictions.filter((pred) => pred.timeToOccurrence <= hours);
        }
        res.json({
            totalPredictions: predictions.length,
            predictions: predictions.sort((a, b) => a.timeToOccurrence - b.timeToOccurrence),
        });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post("/testing/execute/:suiteId", async (req, res) => {
    try {
        ...
        const { executeIntegrationTestSuite } = await import("../integration-intelligence.api");
...
router.get("/testing/reports", async (req, res) => {
    try {
        ...
        const { getIntegrationTestReports } = await import("../integration-intelligence.api");
...
router.post("/testing/schedule", async (req, res) => {
    try {
        ...
        const { scheduleIntegrationTests } = await import("../integration-intelligence.api");
    ...
    try {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                const updatedSystemHealth = updatedHealthReport.individualSystemHealth.find((system) => system.systemId === systemId);
                sendUpdate({
                    type: "health_update",
                    systemId,
...
router.get("/trends/:systemId", async (req, res) => {
    try {
        ...
        const healthReport = await monitorIntegrationHealth();
        let trends = healthReport.performanceTrends.filter((trend) => trend.systemId === systemId);
        if (metricType) {
