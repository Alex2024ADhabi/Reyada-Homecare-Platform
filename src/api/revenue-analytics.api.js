...
import { optimizeServiceMix, } from "./revenue-intelligence.api";
// Revenue Analytics API Functions
// Get comprehensive revenue analytics dashboard
export async function getRevenueAnalyticsDashboard(dateFrom, dateTo) {
    try {
        const db = getDb();
        ...
        const totalRevenue = revenueData.reduce((sum, r) => sum + (r.predicted_net_revenue || 0), 0);
        const revenueGrowth = 0.085; // 8.5% growth
        const marginImprovement = 0.032; // 3.2% margin improvement
        const forecastAccuracy = revenueData.length > 0
            ? revenueData.reduce((sum, r) => sum + (r.historical_accuracy || 0.92), 0) / revenueData.length
            : 0.92;
        ...
        const implementationROI = optimizations.length > 0
...
export async function generateSmartClaimsAnalytics(claimData) {
    try {
        const db = getDb();
...
export async function generatePaymentPrediction(claimData) {
    try {
        const db = getDb();
        ...
        const expectedPaymentAmount = claimData.claimAmount * (0.9 + Math.random() * 0.1);
        ...
        expectedPaymentDate.setDate(expectedPaymentDate.getDate() + 15 + Math.random() * 30);
        const prediction = {
...
export async function createServiceMixOptimization(currentMix, constraints) {
    try {
        const db = getDb();
        ...
        const result = {
            ...
            currentAllocation: currentMix.serviceLines.reduce((acc, line) => {
                acc[line.serviceLineId] = line.currentAllocation;
                return acc;
            }, {}),
            optimizedAllocation: optimization.optimizedAllocation.reduce((acc, alloc) => {
                acc[alloc.serviceLineId] = alloc.optimizedAllocation;
                return acc;
            }, {}),
            expectedRevenueIncrease: optimization.expectedResults.totalRevenueIncrease,
            expectedMarginImprovement: optimization.expectedResults.totalMarginImprovement,
            capacityUtilizationImprovement: optimization.expectedResults.capacityUtilizationImprovement,
            ...
            riskAssessment: {
                ...
                mitigationStrategies: optimization.riskAssessment.map((risk) => risk.mitigationStrategy),
            },
            status: "planned",
        };
...
export async function updateOptimizationResults(optimizationId, actualResults) {
    try {
        const db = getDb();
        const collection = db.collection("service_mix_optimizations");
        await collection.updateOne({ optimization_id: optimizationId }, {
            $set: {
...
export async function getClaimsAnalyticsSummary(dateFrom, dateTo) {
    try {
        const db = getDb();
        const today = new Date().toISOString().split("T")[0];
        const fromDate = dateFrom || today;
        ...
        const averageCleanClaimRate = analytics.length > 0
...
export async function getPaymentPredictionsSummary(dateFrom, dateTo) {
    try {
        const db = getDb();
        const today = new Date().toISOString().split("T")[0];
        const fromDate = dateFrom || today;
        ...
        const averagePaymentProbability = predictions.length > 0
            ? predictions.reduce((sum, p) => sum + p.payment_probability, 0) /
                predictions.length
            : 0;
        const averagePaymentRisk = predictions.length > 0
            ? predictions.reduce((sum, p) => sum + p.payment_risk, 0) /
                predictions.length
            : 0;
        const averageConfidenceScore = predictions.length > 0
...
export async function generateRevenueIntelligenceReport(parameters) {
    try {
        ...
        const dashboard = await getRevenueAnalyticsDashboard(parameters.dateFrom, parameters.dateTo);
        const claimsSummary = await getClaimsAnalyticsSummary(parameters.dateFrom, parameters.dateTo);
        const paymentsSummary = await getPaymentPredictionsSummary(parameters.dateFrom, parameters.dateTo);
