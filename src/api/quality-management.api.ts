...
export interface QualityManagement {
  ...
  compliance_requirements: {
    regulation: "DOH" | "TASNEEF" | "ACHC" | "JAWDA" | "ISO";
...
export interface JAWDAKPITracking {
  _id?: ObjectId;
  kpi_id: string;
  kpi_name: string;
  kpi_category:
  ...
  performance_status: "exceeds" | "meets" | "below" | "critical";
  data_source: string;
...
export interface ComplianceMonitoring {
  ...
  compliance_category: "mandatory" | "recommended" | "best_practice";
  current_status:
    | "compliant"
    | "non_compliant"
    | "partially_compliant"
  ...
  assessment_frequency: "monthly" | "quarterly" | "semi_annually" | "annually";
  responsible_department: string;
  compliance_officer: string;
  evidence_required: {
    document_type: string;
    document_name: string;
    status: "available" | "missing" | "expired" | "under_review";
    expiry_date?: string;
    file_path?: string;
  }[];
...
export interface AuditManagement {
  ...
  audit_title: string;
  audit_scope: string;
  auditing_body: string;
  lead_auditor: string;
  audit_team: string[];
  scheduled_date: string;
  actual_start_date?: string;
  actual_end_date?: string;
  audit_duration_days: number;
  departments_audited: string[];
  standards_audited: string[];
  audit_checklist: {
...
// DOH Ranking Audit Compliance Interfaces
export interface HealthcareFacility {
  ...
  clinicalServices: {
    admissionCriteria: boolean;
    serviceProcess: boolean;
    documentation: boolean;
    malaffiIntegration: boolean;
    specializedServices: boolean;
  };
}

export interface HRComplianceResult {
  governanceStructureHomecare: ComplianceItem;
  currentLicenseOrgChart: ComplianceItem;
  medicalDirector: ComplianceItem;
  facilityGovernance: ComplianceItem;
  staffFilesPrivileging: ComplianceItem;
  tawteenInitiative: ComplianceItem;
}

export interface QualityManagementResult {
  qualityGovernance: ComplianceItem;
  patientIdentification: ComplianceItem;
  effectiveCommunication: ComplianceItem;
  patientAssessment: ComplianceItem;
  medicationManagement: ComplianceItem;
  riskManagementJAWDA: ComplianceItem;
}

export interface ClinicalPracticeResult {
  admissionEligibility: ComplianceItem;
  serviceProcess: ComplianceItem;
  clinicalDocumentation: ComplianceItem;
  specializedServices: ComplianceItem;
  twentyFourHourRestrictions: ComplianceItem;
}

export interface ComplianceItem {
  requirement: string;
  evidenceRequired: string | string[];
  compliance: boolean;
  score: number;
  maxScore: number;
  weight: number;
...
export interface DOHAuditComplianceResult {
  facilityId: string;
  auditDate: Date;
  ...
  auditResults: {
    organizationManagement: {
      humanResources: HRComplianceResult;
      qualityManagement: QualityManagementResult;
      complaintManagement: ComplianceItem;
    };
    medicalRequirements: {
      clinicalPractice: ClinicalPracticeResult;
      medicalRecords: ComplianceItem;
      medication: ComplianceItem;
    };
    infectionControl: {
      surveillance: ComplianceItem;
      safePractices: ComplianceItem;
      nutritionTherapy: ComplianceItem;
      education: ComplianceItem;
      cleaning: ComplianceItem;
      bloodBorneViruses: ComplianceItem;
      wasteManagement: ComplianceItem;
    };
    facilityEquipmentManagement: {
      facilityManagement: ComplianceItem;
      equipmentManagement: ComplianceItem;
      fireSafety: ComplianceItem;
    };
    oshRequirements: ComplianceItem;
    diagnosticServices: {
      laboratoryServices: ComplianceItem;
      pointOfCareTesting: ComplianceItem;
    };
  };
  criticalNonCompliances: string[];
  majorNonCompliances: string[];
  improvementAreas: string[];
  rankingImpact: {
    currentRanking: string;
    potentialRanking: string;
    improvementRequired: number;
  };
}

// Get all quality management records with optional filters
export async function getQualityManagementRecords(
  ...
  try {
    ...
    return records as QualityManagement[];
  } catch (error) {
...
export async function getQualityManagementById(
  id: string,
  ...
  try {
    ...
    return record as QualityManagement | null;
  } catch (error) {
...
export async function createQualityManagementRecord(
  qualityData: Omit<QualityManagement, "_id" | "created_at" | "updated_at">,
): Promise<QualityManagement> {
  try {
    const db = getDb();
    const collection = db.collection("quality_management");

    const newRecord: QualityManagement = {
      ...qualityData,
...
export async function updateQualityManagementRecord(
  id: string,
  updates: Partial<QualityManagement>,
): Promise<QualityManagement | null> {
  try {
    const db = getDb();
    const collection = db.collection("quality_management");
...
export async function getJAWDAKPIRecords(): Promise<JAWDAKPITracking[]> {
  try {
    const db = getDb();
    const collection = db.collection("jawda_kpi_tracking");
    const records = await collection.find({}).toArray();
    return records as JAWDAKPITracking[];
  } catch (error) {
...
export async function createJAWDAKPIRecord(
  kpiData: Omit<JAWDAKPITracking, "_id" | "created_at" | "updated_at">,
): Promise<JAWDAKPITracking> {
  try {
    const db = getDb();
    const collection = db.collection("jawda_kpi_tracking");
    ...
    const newRecord: JAWDAKPITracking = {
      ...enhancedKpiData,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
...
export async function automateKPIDataCollection(kpiData: any): Promise<any> {
  try {
    const db = getDb();
    ...
    switch (kpiData.kpi_category) {
      case "patient_safety":
        actualValue = await calculatePatientSafetyKPI(
          kpiData.kpi_name,
          kpiData.measurement_period,
        );
        dataSource = "automated_clinical_system";
        break;

      case "clinical_effectiveness":
        actualValue = await calculateClinicalEffectivenessKPI(
          kpiData.kpi_name,
          kpiData.measurement_period,
        );
        dataSource = "automated_clinical_system";
      ...
      case "patient_experience":
        actualValue = await calculatePatientExperienceKPI(
          kpiData.kpi_name,
          kpiData.measurement_period,
        );
      ...
      case "operational_efficiency":
        actualValue = await calculateOperationalEfficiencyKPI(
          kpiData.kpi_name,
          kpiData.measurement_period,
        );
      ...
      case "staff_satisfaction":
        actualValue = await calculateStaffSatisfactionKPI(
          kpiData.kpi_name,
          kpiData.measurement_period,
        );
...
async function calculatePatientSafetyKPI(
  kpiName: string,
...
async function calculateClinicalEffectivenessKPI(
  kpiName: string,
  measurementPeriod: string,
): Promise<number> {
  try {
    ...
    switch (kpiName.toLowerCase()) {
      case "patient_improvement_rate":
        ...
        const improvedPatients = assessments.filter(
          (a) => a.improvement_score > 0,
        ).length;
...
async function calculatePatientExperienceKPI(
  kpiName: string,
  measurementPeriod: string,
): Promise<number> {
  try {
    ...
    switch (kpiName.toLowerCase()) {
      case "patient_satisfaction_score":
        const totalScore = surveys.reduce(
          (sum, survey) => sum + (survey.overall_satisfaction || 0),
          0,
        );
        return totalScore / surveys.length;

      case "recommendation_rate":
        const wouldRecommend = surveys.filter(
          (s) => s.would_recommend === true,
        ).length;
...
async function calculateOperationalEfficiencyKPI(
  kpiName: string,
  measurementPeriod: string,
): Promise<number> {
  try {
    ...
    switch (kpiName.toLowerCase()) {
      case "staff_utilization_rate":
        ...
        const totalScheduledHours = attendanceRecords.reduce(
...
async function calculateStaffSatisfactionKPI(
  kpiName: string,
  measurementPeriod: string,
): Promise<number> {
  try {
...
function parseMeasurementPeriod(period: string): [string, string] {
  const now = new Date();
...
function calculateNextUpdateDue(frequency: string): string {
  const now = new Date();
  let nextUpdate: Date;
...
export async function getComplianceMonitoringRecords(): Promise<
...
> {
  try {
    ...
    const records = await collection.find({}).toArray();
    return records as ComplianceMonitoring[];
  } catch (error) {
...
export async function createComplianceMonitoringRecord(
  ...
  try {
    ...
    const newRecord: ComplianceMonitoring = {
      ...complianceData,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
...
export async function getAuditManagementRecords(): Promise<AuditManagement[]> {
  try {
    const db = getDb();
    const collection = db.collection("audit_management");
    const records = await collection.find({}).toArray();
    return records as AuditManagement[];
  } catch (error) {
...
export async function createAuditManagementRecord(
  auditData: Omit<AuditManagement, "_id" | "created_at" | "updated_at">,
): Promise<AuditManagement> {
  try {
    const db = getDb();
    const collection = db.collection("audit_management");

    const newRecord: AuditManagement = {
      ...auditData,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
...
export async function getQualityAnalytics() {
  try {
    ...
    const [qualityRecords, kpiRecords, complianceRecords, auditRecords] =
      await Promise.all([
    ...
    const analytics = {
      ...
      active_quality_initiatives: qualityRecords.filter(
        (r: any) => r.status === "in_progress",
      ).length,
      completed_quality_initiatives: qualityRecords.filter(
        (r: any) => r.status === "completed",
      ).length,
      total_kpis: kpiRecords.length,
      kpis_meeting_target: kpiRecords.filter(
        (k: any) =>
          k.performance_status === "meets" ||
          k.performance_status === "exceeds",
      ).length,
      kpis_below_target: kpiRecords.filter(
        (k: any) =>
          k.performance_status === "below" ||
          k.performance_status === "critical",
      ).length,
      total_compliance_requirements: complianceRecords.length,
      compliant_requirements: complianceRecords.filter(
        (c: any) => c.current_status === "compliant",
      ).length,
      non_compliant_requirements: complianceRecords.filter(
        (c: any) => c.current_status === "non_compliant",
      ).length,
      total_audits: auditRecords.length,
      pending_audits: auditRecords.filter(
        (a: any) => new Date(a.scheduled_date) > new Date(),
      ).length,
      completed_audits: auditRecords.filter((a: any) => a.actual_end_date)
        .length,
      average_compliance_score:
        complianceRecords.length > 0
          ? complianceRecords.reduce(
              (sum: number, c: any) => sum + c.compliance_percentage,
              0,
            ) / complianceRecords.length
          : 0,
      critical_issues: qualityRecords.filter(
        (r: any) => r.priority === "critical",
      ).length,
      overdue_actions: qualityRecords.reduce((count: number, record: any) => {
        return (
          count +
          record.improvement_actions.filter(
            (action: any) =>
...
export async function getComplianceDashboardData() {
  try {
    ...
    const complianceByRegulation = complianceRecords.reduce(
      (acc: any, record: any) => {
    ...
    // Get upcoming audits
    const upcomingAudits = auditRecords.filter(
      (audit: any) =>
...
export async function performAutomatedComplianceCheck(
  regulationType: string = "DOH",
): Promise<{
  ...
  try {
    ...
    if (regulationType === "DOH") {
      ...
      if (patientSafetyScore.score < 80) {
        ...
        recommendations.push(
          "Implement enhanced patient safety protocols and incident reporting",
        );
      }
      ...
      if (documentationScore.score < 85) {
        ...
        recommendations.push(
          "Standardize clinical documentation and ensure completeness",
        );
      }
      ...
      if (staffQualificationScore.score < 90) {
        ...
        recommendations.push(
          "Ensure all staff have current licenses and certifications",
        );
      }
    ...
    const overallScore =
      scoredItems > 0 ? Math.round(totalScore / scoredItems) : 0;
...
async function checkPatientSafetyCompliance(): Promise<{
  score: number;
  issues: string[];
}> {
  try {
    ...
    const medicationErrors = recentIncidents.filter(
      (incident) => incident.incident_type === "medication",
    );
    if (medicationErrors.length > 5) {
      score -= 10;
      issues.push(
        `High medication error rate: ${medicationErrors.length} errors`,
      );
    }
...
async function checkDocumentationCompliance(): Promise<{
  score: number;
  issues: string[];
}> {
  try {
    ...
    const incompleteAssessments = recentAssessments.filter(
      (assessment) => !assessment.completed || !assessment.signature,
    );

    if (incompleteAssessments.length > 0) {
      score -= 20;
      issues.push(
        `${incompleteAssessments.length} incomplete clinical assessments`,
      );
    }
    ...
    const lateDocumentation = recentAssessments.filter((assessment) => {
      const serviceDate = new Date(
        assessment.service_date || assessment.assessment_date,
      );
    ...
    if (lateDocumentation.length > 0) {
      score -= 15;
      issues.push(
        `${lateDocumentation.length} assessments documented late (>24 hours)`,
      );
    }
...
async function checkStaffQualificationCompliance(): Promise<{
  score: number;
  issues: string[];
}> {
  try {
...
async function checkQualityManagementCompliance(): Promise<{
  score: number;
  issues: string[];
}> {
  try {
    ...
    const underperformingKPIs = kpiRecords.filter(
      (kpi) =>
...
async function checkInfectionControlCompliance(): Promise<{
  score: number;
  issues: string[];
}> {
  try {
    ...
    if (infectionIncidents.length > 2) {
      score -= 25;
      issues.push(
        `${infectionIncidents.length} infection-related incidents in last 90 days`,
      );
    }
    ...
    if (handHygieneCompliance < 95) {
      score -= 15;
      issues.push(
        `Hand hygiene compliance below 95%: ${Math.round(handHygieneCompliance)}%`,
      );
    }
...
// Log compliance check results
async function logComplianceCheckResults(results: any): Promise<void> {
  try {
    const db = getDb();
...
export async function prepareDOHAudit(
  auditDate: string,
  auditScope: string[],
): Promise<{
    ...
    const preparationChecklist = [
      {
        ...
        due_date: new Date(
          new Date(auditDate).getTime() - 30 * 24 * 60 * 60 * 1000,
        ).toISOString(),
        priority: "high",
      },
      {
        ...
        due_date: new Date(
          new Date(auditDate).getTime() - 21 * 24 * 60 * 60 * 1000,
        ).toISOString(),
        priority: "critical",
      },
      {
        ...
        due_date: new Date(
          new Date(auditDate).getTime() - 14 * 24 * 60 * 60 * 1000,
        ).toISOString(),
        priority: "critical",
      },
      {
      ...
      },
      {
        ...
        due_date: new Date(
          new Date(auditDate).getTime() - 7 * 24 * 60 * 60 * 1000,
        ).toISOString(),
        priority: "medium",
      },
      {
    ...
    const actionPlan = complianceResults.critical_gaps.map((gap) => ({
      ...
      actions: [
        ...
        },
        {
          ...
          due_date: new Date(
            new Date(gap.deadline).getTime() + 7 * 24 * 60 * 60 * 1000,
          ).toISOString(),
          priority: "high",
...
async function scheduleAuditReminders(
  auditDate: string,
  checklist: any[],
): Promise<void> {
  try {
...
export class DOHRankingAuditEngine {
  async validateDOHAuditCompliance(
    facility: HealthcareFacility,
  ): Promise<DOHAuditComplianceResult> {
    try {
      ...
      return {
        ...
        criticalNonCompliances:
          this.identifyCriticalNonCompliances(auditCompliance),
  ...
  private async validateHumanResourcesCompliance(
    facility: HealthcareFacility,
  ): Promise<HRComplianceResult> {
    ...
    const staffFilesCompliance =
      await this.checkStaffFilesPrivileging(facility);
    ...
    return {
      ...
      governanceStructureHomecare: {
        requirement:
          "Meet minimum requirement of one (1) Physician and twenty-five (25) Registered Nurses and appropriate skill mix",
      ...
      currentLicenseOrgChart: {
        requirement:
          "Current DOH License displayed, approved organization chart, valid professional licenses",
        evidenceRequired: [
      ...
      medicalDirector: {
        requirement:
          "Home Healthcare service provider must have a Medical Director",
      ...
      facilityGovernance: {
        requirement:
          "Defined scope of services, governance protocol, policies & SOPs",
        evidenceRequired: [
      ...
      tawteenInitiative: {
        requirement:
          "Meet minimum Tawteen targets: 1% healthcare workforce, 2% administrative workforce",
        evidenceRequired: [
  ...
  private async validateQualityManagement(
    facility: HealthcareFacility,
  ): Promise<QualityManagementResult> {
    const qualityGovernanceCompliance =
      await this.checkQualityGovernance(facility);
    const patientIdCompliance = await this.checkPatientIdentification(facility);
    const communicationCompliance =
      await this.checkEffectiveCommunication(facility);
    ...
    const riskManagementCompliance =
      await this.checkRiskManagementJAWDA(facility);

    return {
      // QM001 - Quality Management & Governance Structure (Weight: 2)
      qualityGovernance: {
        requirement:
          "Healthcare facility establishes quality management & governance structure",
        evidenceRequired: [
          "Quality Manual",
      ...
      patientIdentification: {
        requirement:
          "Accurate patient identification with at least two identifiers",
        evidenceRequired: [
      ...
      effectiveCommunication: {
        requirement:
          "Improve effectiveness of communication among healthcare providers",
        evidenceRequired: [
      ...
      patientAssessment: {
        requirement:
          "Process for patient assessment and frequency of re-assessment",
        evidenceRequired: [
  ...
  private async validateClinicalPractice(
    facility: HealthcareFacility,
  ): Promise<ClinicalPracticeResult> {
    ...
    const documentationCompliance =
      await this.checkClinicalDocumentation(facility);
    const specializedServicesCompliance =
      await this.checkSpecializedServices(facility);
    const twentyFourHourCompliance =
      await this.checkTwentyFourHourRestrictions(facility);
    ...
    return {
      ...
      serviceProcess: {
        requirement:
          "Compliance with homecare service process, renewal, and discharge criteria",
        evidenceRequired: [
      ...
      clinicalDocumentation: {
        requirement:
          "Complete clinical documentation including Malaffi integration",
        evidenceRequired: [
      ...
      specializedServices: {
        requirement:
          "Specialized services compliance including controlled medications",
        evidenceRequired: [
      ...
      twentyFourHourRestrictions: {
        requirement:
          "No nursing staff housed at patient residence unless 24-hour service identified",
        evidenceRequired: [
  ...
  private async checkMinimumStaffing(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return (
      facility.staffing.physicians >= 1 &&
      facility.staffing.registeredNurses >= 25
    );
  }

  private async checkLicenseAndOrgChart(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return (
      facility.governance.organizationChart && facility.licenseNumber !== ""
    );
  }

  private async checkMedicalDirector(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return (
      facility.medicalDirector !== undefined &&
      facility.medicalDirector.licenseNumber !== ""
    );
  }

  private async checkFacilityGovernance(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return (
      facility.governance.scopeOfServices &&
      facility.governance.policiesSOPs &&
      facility.governance.patientRights
    );
  }

  private async checkStaffFilesPrivileging(
    facility: HealthcareFacility,
  ): Promise<boolean> {
  ...
  private async checkTawteenCompliance(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    const healthcareEmiratiPercentage =
      (facility.staffing.emiratiStaff / facility.staffing.totalStaff) * 100;
    const adminEmiratiPercentage =
      (facility.staffing.emiratiStaff / facility.staffing.administrativeStaff) *
      100;
    return healthcareEmiratiPercentage >= 1 && adminEmiratiPercentage >= 2;
  }

  private async checkQualityGovernance(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return (
      facility.qualityManagement.qualityManual &&
      facility.qualityManagement.riskManagement
    );
  }

  private async checkPatientIdentification(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return facility.qualityManagement.patientSafety;
  }

  private async checkEffectiveCommunication(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return facility.qualityManagement.patientSafety;
  }

  private async checkPatientAssessment(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return facility.clinicalServices.admissionCriteria;
  }

  private async checkMedicationManagement(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return facility.qualityManagement.patientSafety;
  }

  private async checkRiskManagementJAWDA(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return (
      facility.qualityManagement.riskManagement &&
  ...
  private async checkAdmissionEligibility(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return facility.clinicalServices.admissionCriteria;
  }

  private async checkServiceProcess(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return facility.clinicalServices.serviceProcess;
  }

  private async checkClinicalDocumentation(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return (
      facility.clinicalServices.documentation &&
      facility.clinicalServices.malaffiIntegration
    );
  }

  private async checkSpecializedServices(
    facility: HealthcareFacility,
  ): Promise<boolean> {
    return facility.clinicalServices.specializedServices;
  }

  private async checkTwentyFourHourRestrictions(
    facility: HealthcareFacility,
  ): Promise<boolean> {
  ...
  private async validateComplaintManagement(
    facility: HealthcareFacility,
  ...
  private async validateMedicalRecords(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateInfectionSurveillance(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateSafePractices(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateNutritionTherapy(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateInfectionEducation(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateEnvironmentalCleaning(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateBloodBorneManagement(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateWasteManagement(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateFacilityManagement(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateEquipmentManagement(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateFireSafety(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateOSHRequirements(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validateLaboratoryServices(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private async validatePOCTesting(
    facility: HealthcareFacility,
  ): Promise<ComplianceItem> {
    return {
  ...
  private calculateOverallAuditScore(auditCompliance: any): number {
    try {
      let totalScore = 0;
      let maxScore = 0;
  ...
  private calculateWeightedScore(auditCompliance: any): number {
    try {
      let weightedScore = 0;

      const calculateWeightedSectionScore = (section: any) => {
        if (typeof section === "object" && section !== null) {
          if (
            typeof section.score === "number" &&
  ...
  private identifyCriticalNonCompliances(auditCompliance: any): string[] {
    const criticalNonCompliances: string[] = [];

    const checkCriticalCompliance = (section: any, sectionName: string) => {
      if (typeof section === "object" && section !== null) {
        if (
          section.compliance === false &&
          (section.weight >= 3 || section.maxScore >= 300)
        ) {
          criticalNonCompliances.push(
            `${sectionName}: ${section.requirement || "Critical requirement not met"}`,
          );
        } else {
  ...
  private identifyMajorNonCompliances(auditCompliance: any): string[] {
    const majorNonCompliances: string[] = [];

    const checkMajorCompliance = (section: any, sectionName: string) => {
      if (typeof section === "object" && section !== null) {
        if (
          section.compliance === false &&
          (section.weight === 2 ||
          ...
          majorNonCompliances.push(
            `${sectionName}: ${section.requirement || "Major requirement not met"}`,
          );
        } else {
          Object.entries(section).forEach(([key, value]) => {
            if (
  ...
  private identifyImprovementAreas(auditCompliance: any): string[] {
    const improvementAreas: string[] = [];

    const checkImprovementAreas = (section: any, sectionName: string) => {
      if (typeof section === "object" && section !== null) {
        if (
          ...
          if (compliancePercentage < 80) {
            improvementAreas.push(
              `${sectionName}: ${section.requirement || "Requirement needs improvement"} (${Math.round(compliancePercentage)}% compliance)`,
            );
          }
        } else {
          Object.entries(section).forEach(([key, value]) => {
  ...
  private calculateRankingImpact(weightedScore: number): {
    currentRanking: string;
    potentialRanking: string;
    improvementRequired: number;
  } {
    ...
    let potentialRanking: string;
    let improvementRequired: number;
...
// DOH Audit Compliance Database Schema Interfaces
export interface DOHAuditCompliance {
  audit_id?: string;
  facility_id: string;
  audit_date: string;
  audit_type: "annual_ranking" | "regulatory" | "follow_up";
  auditor_name?: string;
  audit_reference?: string;
  total_weighted_score: number;
  max_possible_score: number;
  compliance_percentage: number;
  ranking_grade?: "A+" | "A" | "B+" | "B" | "C+" | "C" | "D";
  organization_management_score: number;
  medical_requirements_score: number;
  infection_control_score: number;
  facility_equipment_score: number;
  osh_requirements_score: number;
  diagnostic_services_score: number;
  overall_compliant: boolean;
  critical_non_compliances: number;
  major_non_compliances: number;
  minor_non_compliances: number;
  improvement_plan_required: boolean;
  corrective_action_deadline?: string;
  follow_up_audit_scheduled?: string;
  license_renewal_impact?: string;
  accreditation_impact?: string;
  public_ranking_impact?: string;
  created_at: string;
  updated_at: string;
}

export interface DOHAuditRequirement {
  requirement_id?: string;
  audit_id: string;
  requirement_code: string;
  ...
  corrective_actions_required: any[];
  requires_follow_up: boolean;
  follow_up_deadline?: string;
  follow_up_completed: boolean;
}

export interface DOHAuditEvidence {
  evidence_id?: string;
  requirement_id: string;
  evidence_type: string;
  evidence_description?: string;
  evidence_file_path?: string;
  evidence_format: "document" | "screenshot" | "report" | "policy";
...
export async function performDOHRankingAudit(
  facilityId: string,
): Promise<DOHAuditComplianceResult> {
  try {
    const db = getDb();
...
export async function getDOHAuditHistory(
  facilityId: string,
): Promise<DOHAuditCompliance[]> {
  try {
    const db = getDb();
    ...
    return auditHistory as DOHAuditCompliance[];
  } catch (error) {
...
export async function saveDOHAuditCompliance(
  auditResult: DOHAuditComplianceResult,
  facilityId: string,
): Promise<string> {
  try {
    const db = getDb();
    ...
    const rankingGrade = determineRankingGrade(
      auditResult.compliancePercentage,
    );

    // Create main audit compliance record
    const auditCompliance: DOHAuditCompliance = {
      ...
      corrective_action_deadline:
        auditResult.compliancePercentage < 80
          ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
              .toISOString()
              .split("T")[0]
          : undefined,
      follow_up_audit_scheduled:
        auditResult.compliancePercentage < 70
          ? new Date(Date.now() + 90 * 24 * 60 * 60 * 1000)
              .toISOString()
              .split("T")[0]
          : undefined,
      license_renewal_impact:
        auditResult.compliancePercentage < 60 ? "at_risk" : "compliant",
...
function calculateCategoryScores(auditResult: DOHAuditComplianceResult): {
  organizationManagement: number;
  medicalRequirements: number;
  infectionControl: number;
  facilityEquipment: number;
  oshRequirements: number;
  ...
  return {
    organizationManagement: calculateSectionScore(
      auditResult.auditResults?.organizationManagement,
    ),
    medicalRequirements: calculateSectionScore(
      auditResult.auditResults?.medicalRequirements,
    ),
    infectionControl: calculateSectionScore(
      auditResult.auditResults?.infectionControl,
    ),
    facilityEquipment: calculateSectionScore(
      auditResult.auditResults?.facilityEquipmentManagement,
    ),
    oshRequirements: auditResult.auditResults?.oshRequirements?.score || 0,
    diagnosticServices: calculateSectionScore(
      auditResult.auditResults?.diagnosticServices,
    ),
  };
}

// Determine ranking grade based on compliance percentage
function determineRankingGrade(
  percentage: number,
): "A+" | "A" | "B+" | "B" | "C+" | "C" | "D" {
...
async function saveAuditRequirements(
  auditResult: DOHAuditComplianceResult,
  auditId: string,
  requirementsCollection: any,
): Promise<void> {
  try {
    const requirements: DOHAuditRequirement[] = [];
    ...
    const processRequirements = (
      ...
      Object.entries(section).forEach(([key, item]: [string, any]) => {
    ...
    if (auditResult.auditResults?.organizationManagement?.humanResources) {
      processRequirements(
        auditResult.auditResults.organizationManagement.humanResources,
        "HR",
        "Human Resources",
      );
    }
    ...
    if (auditResult.auditResults?.organizationManagement?.qualityManagement) {
      processRequirements(
        auditResult.auditResults.organizationManagement.qualityManagement,
        "QM",
        "Quality Management",
      );
    }
    ...
    if (auditResult.auditResults?.medicalRequirements?.clinicalPractice) {
      processRequirements(
        auditResult.auditResults.medicalRequirements.clinicalPractice,
        "CP",
        "Clinical Practice",
      );
    }
    ...
    if (auditResult.auditResults?.medicalRequirements?.medicalRecords) {
      const medicalRecords =
        auditResult.auditResults.medicalRequirements.medicalRecords;
      if (typeof medicalRecords === "object" && medicalRecords.requirement) {
        ...
        requirements.push({
          ...
          max_score:
            typeof medicalRecords.maxScore === "number"
              ? medicalRecords.maxScore
              : 0,
          weight:
            typeof medicalRecords.weight === "number"
              ? medicalRecords.weight
              : 1,
          achieved_score:
            typeof medicalRecords.score === "number" ? medicalRecords.score : 0,
          weighted_score:
            typeof medicalRecords.score === "number" ? medicalRecords.score : 0,
...
export async function getDOHAuditRequirements(
  auditId: string,
): Promise<DOHAuditRequirement[]> {
  try {
    const db = getDb();
    const requirementsCollection = db.collection("doh_audit_requirements");
    ...
    return requirements as DOHAuditRequirement[];
  } catch (error) {
...
export async function submitDOHAuditEvidence(
  evidenceData: Omit<DOHAuditEvidence, "evidence_id" | "submitted_date">,
): Promise<string> {
  try {
    const db = getDb();
    const evidenceCollection = db.collection("doh_audit_evidence");

    const evidence: DOHAuditEvidence = {
      ...evidenceData,
...
export async function getDOHAuditEvidence(
  requirementId: string,
): Promise<DOHAuditEvidence[]> {
  try {
    const db = getDb();
    const evidenceCollection = db.collection("doh_audit_evidence");
    ...
    return evidence as DOHAuditEvidence[];
  } catch (error) {
...
export async function updateDOHRequirementCompliance(
  requirementId: string,
  updates: Partial<DOHAuditRequirement>,
): Promise<void> {
  try {
    const db = getDb();
    const requirementsCollection = db.collection("doh_audit_requirements");
...
export async function getDOHAuditDashboardData(facilityId: string): Promise<{
  currentAudit: DOHAuditCompliance | null;
  auditHistory: DOHAuditCompliance[];
  pendingRequirements: DOHAuditRequirement[];
  complianceMetrics: {
  ...
  try {
    ...
    const currentAudit = (await auditCollection.findOne(
      { facility_id: facilityId },
      { sort: { audit_date: -1 } },
    )) as DOHAuditCompliance | null;
    ...
    const auditHistory = (await auditCollection
      ...
      .sort({ audit_date: -1 })
      .limit(12)
      .toArray()) as DOHAuditCompliance[];

    // Get pending requirements - handle case where audit_id might be ObjectId
    let pendingRequirements: DOHAuditRequirement[] = [];
    if (currentAudit) {
      try {
        const auditIdToUse =
          currentAudit.audit_id || currentAudit._id?.toString() || "";
        if (auditIdToUse) {
          pendingRequirements = (await requirementsCollection
            ...
            .toArray()) as DOHAuditRequirement[];
        }
      } catch (reqError) {
    ...
    const complianceMetrics = {
      overallCompliance:
        typeof currentAudit?.compliance_percentage === "number"
          ? currentAudit.compliance_percentage
          : 0,
      categoryBreakdown: currentAudit
        ? {
            organizationManagement:
              typeof currentAudit.organization_management_score === "number"
                ? currentAudit.organization_management_score
                : 0,
            medicalRequirements:
              typeof currentAudit.medical_requirements_score === "number"
                ? currentAudit.medical_requirements_score
                : 0,
            infectionControl:
              typeof currentAudit.infection_control_score === "number"
                ? currentAudit.infection_control_score
                : 0,
            facilityEquipment:
              typeof currentAudit.facility_equipment_score === "number"
                ? currentAudit.facility_equipment_score
                : 0,
            oshRequirements:
              typeof currentAudit.osh_requirements_score === "number"
                ? currentAudit.osh_requirements_score
                : 0,
            diagnosticServices:
              typeof currentAudit.diagnostic_services_score === "number"
...
export async function getRealTimeComplianceScore(): Promise<{
  overall_score: number;
  category_scores: any[];
...
export async function getDOHComplianceDashboard(): Promise<{
  realTimeMetrics: {
  ...
  try {
    ...
    const alerts = {
      critical: complianceResults.critical_gaps.map((gap) => ({
        ...
        message:
          gap.gap_description || `Critical compliance issue in ${gap.category}`,
...
async function calculateDocumentationCompliance(): Promise<number> {
  try {
    const db = getDb();
    const assessmentsCollection = db.collection("clinical_assessments");
    ...
    if (recentAssessments.length === 0) return 85; // Default score

    const compliantDocs = recentAssessments.filter(
      (assessment) => assessment.completed && assessment.signature,
    ).length;
...
async function calculatePatientSafetyScore(): Promise<number> {
  try {
    const db = getDb();
    const incidentsCollection = db.collection("incident_reports");
...
async function calculateClinicalQualityScore(): Promise<number> {
  try {
    const db = getDb();
    const kpiCollection = db.collection("jawda_kpi_tracking");
    ...
    if (clinicalKPIs.length === 0) return 88; // Default score

    const averagePerformance =
      clinicalKPIs.reduce((sum, kpi) => {
...
async function calculateRegulatoryAdherence(): Promise<number> {
  try {
    const db = getDb();
    const complianceCollection = db.collection("compliance_monitoring");
    ...
    if (complianceRecords.length === 0) return 82; // Default score
...
async function getComplianceHistory(): Promise<
  {
    date: string;
    overall_score: number;
    category_scores: Record<string, number>;
...
async function getNonCompliantDocuments(): Promise<
  {
    document_type: string;
    patient_id?: string;
    clinician: string;
