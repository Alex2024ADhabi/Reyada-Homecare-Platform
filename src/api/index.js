...
app.use("/api/communication/committee", require("./communication.api").committeeAPI);
app.use("/api/communication/dashboard", require("./communication.api").dashboardAPI);
app.use("/api/communication/governance", require("./communication.api").governanceAPI);
...
app.get("/api/compliance/status", async (req, res) => {
    try {
        const { performRealTimeComplianceCheck } = await import("./administrative-integration.api");
...
app.post("/api/emergency/notify", async (req, res) => {
    try {
        ...
        if (severity === "critical" || incident_type === "patient_safety") {
            ...
            const { createIncidentReport } = await import("./incident-management.api");
