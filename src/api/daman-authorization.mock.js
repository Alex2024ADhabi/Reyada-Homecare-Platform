...
export const mockDamanAuthorizationAPI = {
    ...
    getAuthorizationByReferenceNumber: async (referenceNumber) => {
        const authorization = mockAuthorizations.find((auth) => auth.referenceNumber === referenceNumber);
        if (!authorization)
            throw new Error(`Authorization with reference number ${referenceNumber} not found`);
        return Promise.resolve(authorization);
    },
    ...
    updateAuthorization: async (id, authData) => {
    ...
    uploadDocuments: async (id, documents) => {
        ...
        const uploadedDocumentTypes = updatedAuthorization.documents.map((doc) => doc.documentType);
        const allRequiredDocumentsUploaded = requiredDocumentTypes.every((type) => uploadedDocumentTypes.includes(type));
    ...
    validateDocument: async (id, documentIndex, validationResult) => {
        const index = mockAuthorizations.findIndex((auth) => auth._id === id);
        if (index === -1)
            throw new Error(`Authorization with ID ${id} not found`);
        ...
        const hasInvalidDocuments = updatedAuthorization.documents.some((doc) => doc.status === "Invalid");
        if (hasInvalidDocuments) {
            updatedAuthorization.documents_complete = false;
            updatedAuthorization.ready_for_submission = false;
            if (updatedAuthorization.authorization_status !== "Additional Info Required") {
    ...
    startQAReview: async (id, reviewerName) => {
        const index = mockAuthorizations.findIndex((auth) => auth._id === id);
        if (index === -1)
            throw new Error(`Authorization with ID ${id} not found`);
        const updatedAuthorization = {
    ...
    completeQAReview: async (id, reviewData) => {
        const index = mockAuthorizations.findIndex((auth) => auth._id === id);
        if (index === -1)
            throw new Error(`Authorization with ID ${id} not found`);
        const updatedAuthorization = {
        ...
        if (reviewData.additionalDocumentsRequired &&
        ...
        if (updatedAuthorization.authorization_status === "Approved") {
            ...
            expirationDate.setDate(expirationDate.getDate() + updatedAuthorization.requestedDuration);
    ...
    getSubmissionStatistics: async () => {
        return Promise.resolve({
            ...
            approvalRate: (mockAuthorizations.filter((a) => a.authorization_status === "Approved")
                .length /
            ...
            submissionsByStatus: {
                Pending: mockAuthorizations.filter((a) => a.authorization_status === "Pending").length,
                InReview: mockAuthorizations.filter((a) => a.authorization_status === "In Review").length,
                AdditionalInfoRequired: mockAuthorizations.filter((a) => a.authorization_status === "Additional Info Required").length,
                Approved: mockAuthorizations.filter((a) => a.authorization_status === "Approved").length,
                Denied: mockAuthorizations.filter((a) => a.authorization_status === "Denied").length,
            },
        });
    },
