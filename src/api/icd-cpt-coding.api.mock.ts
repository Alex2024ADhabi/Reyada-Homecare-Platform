import { ICDCPTCodingRecord } from "./icd-cpt-coding.api";

// Mock data for ICD & CPT coding records
const mockRecords: ICDCPTCodingRecord[] = [
  {
    id: "1",
    patient_id: "P12345",
    mrn: "MRN12345",
...
export const mockICDCPTCodingAPI = {
  ...
  getRecordsByPatientId: async (patientId: string) =>
    Promise.resolve(
  ...
  createRecord: async (recordData: Partial<ICDCPTCodingRecord>) => {
    const newRecord = {
      id: `${Date.now()}`,
      ...
      physician_response_received:
        recordData.physician_response_received || false,
      qa_audit_completed: recordData.qa_audit_completed || false,
      peer_review_completed: recordData.peer_review_completed || false,
      authorization_code_alignment_checked:
        recordData.authorization_code_alignment_checked || false,
      coding_status: recordData.coding_status || "In Progress",
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    mockRecords.push(newRecord as ICDCPTCodingRecord);
    return Promise.resolve(newRecord as ICDCPTCodingRecord);
  },

  updateRecord: async (id: string, recordData: Partial<ICDCPTCodingRecord>) => {
  ...
  submitAuthorizationCheck: async (
    id: string,
  ...
  resolveDiscrepancy: async (id: string) => {
    const index = mockRecords.findIndex((record) => record.id === id);
    if (index === -1) throw new Error(`Record with ID ${id} not found`);
  ...
  performGapAnalysis: async (patientId: string) => {
  ...
  enhancedMedicalRecordReview: async (
    id: string,
  ...
  getCodingMetrics: async (patientId?: string) => {
    ...
    const metrics = {
      ...
      completedRecords: filteredRecords.filter(
        (r) => r.coding_status === "Completed",
      ).length,
