      ...
      initiatives: [
        {
          ...
          description:
            "Recruitment and training program for Emirati nursing graduates",
          status: "active",
          expectedHires: 8,
          timeline: "6 months",
        },
  ...
  try {
    ...
    const report = {
      ...
      complianceCalculation: {
        ...
        percentage: (
          (reportData.emiratizationData.emiratiEmployees /
  ...
  try {
    ...
    for (const employee of workforceData.employees) {
      if (!employee.employeeId || !employee.nationality || !employee.category) {
        return res.status(400).json({
          error:
            "Each employee must have employeeId, nationality, and category",
        });
      }
    ...
    const emiratiEmployees = workforceData.employees.filter(
      (emp) => emp.nationality === "UAE",
    ).length;
    const emiratizationPercentage = (
      (emiratiEmployees / totalEmployees) *
      100
    ).toFixed(2);
    ...
    categories.forEach((category) => {
      const categoryEmployees = workforceData.employees.filter(
        (emp) => emp.category === category,
      );
      const categoryEmiratis = categoryEmployees.filter(
        (emp) => emp.nationality === "UAE",
      );
    ...
    res.json({
      ...
      complianceStatus:
        parseFloat(emiratizationPercentage) >= 35
          ? "compliant"
          : "non-compliant",
    });
  } catch (error) {
