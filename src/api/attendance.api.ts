...
export async function getAttendanceRecords(
  ...
  try {
    ...
    return records as StaffAttendance[];
  } catch (error) {
...
export async function getAttendanceById(
  ...
  try {
    ...
    return record as StaffAttendance | null;
  } catch (error) {
...
export async function createAttendanceRecord(
  attendanceData: Omit<StaffAttendance, "_id" | "created_at" | "updated_at">,
): Promise<StaffAttendance> {
  try {
    const db = getDb();
    const collection = db.collection("staff_attendance");
...
export async function updateAttendanceRecord(
  id: string,
  updates: Partial<StaffAttendance>,
): Promise<StaffAttendance | null> {
  try {
    const db = getDb();
    const collection = db.collection("staff_attendance");
...
export async function clockIn(
  ...
  try {
    ...
    const scheduledStart = new Date(
      `${today}T${employeeInfo.scheduled_start}:00`,
    );
    ...
    const lateMinutes = isLate
      ? Math.floor(
          (actualStart.getTime() - gracePeriodEnd.getTime()) / (1000 * 60),
        )
      : 0;
    ...
    if (existingRecord) {
      ...
      return { ...attendanceData, _id: existingRecord._id } as StaffAttendance;
    } else {
      // Create new record
...
export async function getTimesheetSummaries(
  ...
  try {
    ...
    return summaries as TimesheetSummary[];
  } catch (error) {
...
export async function generateTimesheetSummary(
  employee_id: string,
  pay_period_start: string,
  pay_period_end: string,
): Promise<TimesheetSummary> {
  try {
    ...
    const daysLate = attendanceRecords.filter(
      (record) => record.late_arrival,
    ).length;
    const totalLateMinutes = attendanceRecords.reduce(
      (sum, record) => sum + (record.late_minutes || 0),
      0,
    );
    const sickDays = attendanceRecords.filter(
      (record) => record.status === "sick_leave",
    ).length;
    const vacationDays = attendanceRecords.filter(
    ...
    const summary: Omit<TimesheetSummary, "_id" | "created_at" | "updated_at"> =
      {
        employee_id,
    ...
    if (existingSummary) {
      ...
      return { ...summary, _id: existingSummary._id } as TimesheetSummary;
    } else {
      // Create new summary
      ...
      return { ...summary, _id: result.insertedId } as TimesheetSummary;
    }
  } catch (error) {
...
export async function approveTimesheet(
  ...
  try {
    ...
    return updatedSummary as TimesheetSummary | null;
  } catch (error) {
...
export async function sendLateArrivalNotification(
  ...
  try {
    ...
    const notification = {
      ...
      late_minutes: lateMinutes,
      severity:
        lateMinutes > 30 ? "critical" : lateMinutes > 15 ? "high" : "medium",
    ...
    console.log(
      `CRITICAL LATE ARRIVAL ALERT: Employee ${employee_id} (${employeeInfo.employee_name}) is ${lateMinutes} minutes late - Supervisor ${supervisorId} notified within 15-minute requirement`,
    );
  } catch (error) {
...
// CRITICAL: DOH notification for excessive lateness
export async function sendDOHNotification(
  employee_id: string,
  lateMinutes: number,
  employeeInfo: any,
): Promise<void> {
  try {
    ...
    console.log(
      `DOH notification queued for employee ${employee_id} - ${lateMinutes} minutes late`,
    );
  } catch (error) {
...
export async function escalateLateArrival(
  employee_id: string,
  lateMinutes: number,
  employeeInfo: any,
  supervisorId: string,
...
export async function flagAttendanceForReview(
  employee_id: string,
  lateMinutes: number,
): Promise<void> {
  try {
...
// Get staff category-specific timesheet data
export interface StaffCategoryTimesheet {
  _id?: ObjectId;
  employee_id: string;
  employee_name: string;
  staff_category:
  ...
  patient_visit_time?: number; // For clinical staff
  travel_time?: number; // For field staff
  equipment_setup_time?: number; // For therapists
  equipment_breakdown_time?: number; // For therapists
  break_time?: number; // All staff
  break_duration_minutes?: number; // Specific break durations
  vaccination_count?: number; // For Etihad vaccination staff
  admin_tasks_completed?: string[]; // For admin staff
  driving_distance_km?: number; // For drivers
  vehicle_inspection_time?: number; // For drivers
  created_at: string;
  updated_at: string;
}

// Create staff category-specific timesheet
export async function createStaffCategoryTimesheet(
  timesheetData: Omit<
    StaffCategoryTimesheet,
    "_id" | "created_at" | "updated_at"
  >,
): Promise<StaffCategoryTimesheet> {
  try {
    const db = getDb();
    const collection = db.collection("staff_category_timesheets");

    const newTimesheet: StaffCategoryTimesheet = {
      ...timesheetData,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
...
export async function getStaffCategoryTimesheets(
  filters: {
    employee_id?: string;
  ...
  try {
    ...
    if (filters.staff_category) query.staff_category = filters.staff_category;
    if (filters.date_from || filters.date_to) {
      query.date = {};
      if (filters.date_from) query.date.$gte = filters.date_from;
      if (filters.date_to) query.date.$lte = filters.date_to;
    ...
    return timesheets as StaffCategoryTimesheet[];
  } catch (error) {
...
export async function createEtihadVaccinationTimesheet(timesheetData: {
  ...
  vaccination_sessions: {
    ...
    adverse_reactions_handled: number;
  ...
  cleanup_time: number;
  break_time: number;
  administrative_tasks: string[];
}): Promise<StaffCategoryTimesheet> {
  try {
    const totalVaccinations = timesheetData.vaccination_sessions.reduce(
      (sum, session) => sum + session.vaccines_administered,
      0,
    );

    const totalSessionTime = timesheetData.vaccination_sessions.reduce(
      (sum, session) => {
        const start = new Date(
          `${timesheetData.date}T${session.session_start}`,
        );
...
export async function createICONTimesheet(timesheetData: {
  ...
  travel_between_visits: number;
  administrative_time: number;
  break_time: number;
  equipment_maintenance_time: number;
}): Promise<StaffCategoryTimesheet> {
  try {
    const totalPatientTime = timesheetData.patient_visits.reduce(
      (sum, visit) => {
...
export async function createRHHCSAdminTimesheet(timesheetData: {
  ...
  try {
    const totalAdminTime = timesheetData.administrative_tasks.reduce(
      (sum, task) => {
    ...
    const totalMeetingTime = timesheetData.meetings_attended.reduce(
      (sum, meeting) => sum + meeting.duration,
      0,
    );

    const taskSummary = timesheetData.administrative_tasks.map(
      (task) => `${task.task_name} (${task.task_category})`,
    );
...
export async function createRHHCSNurseTimesheet(timesheetData: {
  ...
  patient_visits: {
    ...
    visit_start: string;
    visit_end: string;
    nursing_procedures: string[];
    medication_administration: boolean;
    wound_care: boolean;
    vital_signs_monitoring: boolean;
  ...
  try {
    ...
    const proceduresSummary = timesheetData.patient_visits.flatMap(
      (visit) => visit.nursing_procedures,
    );
...
export async function createRHHCSSMOTimesheet(timesheetData: {
  ...
  patient_consultations: {
    ...
    consultation_type: "Initial" | "Follow-up" | "Emergency" | "Specialist";
  ...
  try {
    const totalConsultationTime = timesheetData.patient_consultations.reduce(
      (sum, consultation) => {
        const start = new Date(
          `${timesheetData.date}T${consultation.consultation_start}`,
        );
        const end = new Date(
          `${timesheetData.date}T${consultation.consultation_end}`,
        );
        return sum + (end.getTime() - start.getTime()) / (1000 * 60);
      },
      0,
    ...
    const totalAdminTime = timesheetData.administrative_duties.reduce(
      (sum, duty) => sum + duty.duration,
      0,
    );

    const adminTasks = timesheetData.administrative_duties.map(
      (duty) => duty.task,
    );
...
export async function createRHHCSTherapistDriverTimesheet(timesheetData: {
  ...
  driving_assignments?: {
    ...
    distance_km: number;
    passengers: number;
    vehicle_inspection_time: number;
  }[];
  travel_time: number;
  break_time: number;
  equipment_maintenance_time?: number;
  ...
  try {
    ...
    let adminTasks: string[] = [];

    if (timesheetData.role === "Therapist" && timesheetData.therapy_sessions) {
      patientVisitTime = timesheetData.therapy_sessions.reduce(
        (sum, session) => {
          const start = new Date(
            `${timesheetData.date}T${session.session_start}`,
          );
      ...
      equipmentSetupTime = timesheetData.therapy_sessions.reduce(
        (sum, session) => sum + session.setup_time,
        0,
      );

      equipmentBreakdownTime = timesheetData.therapy_sessions.reduce(
        (sum, session) => sum + session.breakdown_time,
        0,
      );
    ...
    if (timesheetData.role === "Driver" && timesheetData.driving_assignments) {
      drivingDistance = timesheetData.driving_assignments.reduce(
        (sum, assignment) => sum + assignment.distance_km,
        0,
      );

      vehicleInspectionTime = timesheetData.driving_assignments.reduce(
        (sum, assignment) => sum + assignment.vehicle_inspection_time,
        0,
      );
...
export async function getAttendanceComplianceReport(filters: {
  ...
  disciplinary_actions: {
    ...
    disciplinary_actions_taken: number;
  };
  staff_category_breakdown: Record<
    string,
    {
      total_timesheets: number;
      average_patient_visit_time: number;
  ...
  try {
    const db = getDb();
    const attendanceCollection = db.collection("staff_attendance");
    const categoryTimesheetsCollection = db.collection(
      "staff_category_timesheets",
    );
    const escalationsCollection = db.collection("attendance_escalations");
    ...
    const totalClockIns = attendanceRecords.filter(
      (r) => r.actual_start,
    ).length;
    const onTimeArrivals = attendanceRecords.filter(
      (r) => !r.late_arrival,
    ).length;
    const withinGracePeriod = attendanceRecords.filter(
      (r) => r.late_arrival && r.late_minutes <= 15,
    ).length;
    ...
    const flaggedForReview = attendanceRecords.filter(
      (r) => r.flagged_for_review,
    ).length;
    const escalatedCases = escalations.length;
    const disciplinaryActions = escalations.filter(
      (e) => e.disciplinary_action_recommended,
    ).length;
    ...
    categories.forEach((category) => {
      const categoryData = categoryTimesheets.filter(
        (t) => t.staff_category === category,
      );
      const totalTimesheets = categoryData.length;
      const avgPatientVisitTime =
        totalTimesheets > 0
          ? categoryData.reduce(
              (sum, t) => sum + (t.patient_visit_time || 0),
              0,
            ) / totalTimesheets
          : 0;
      const avgTravelTime =
