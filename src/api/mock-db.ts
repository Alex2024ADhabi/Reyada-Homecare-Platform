...
function createMockCollection(name: string): MockCollection {
  return {
      ...
      return collections[name].filter((item) => {
        for (const [key, value] of Object.entries(query)) {
          if (key === "_id") {
            ...
            if (itemIdStr !== idStr) return false;
    ...
    updateOne: async (query: any, update: any, options: any = {}) => {
      ...
      if (query._id) {
        const idStr =
          typeof query._id === "string" ? query._id : query._id.toString();
      ...
      if (index !== -1) {
        ...
        if (update.$pull) {
          for (const [key, condition] of Object.entries(update.$pull)) {
            if (Array.isArray(updatedDoc[key])) {
              updatedDoc[key] = updatedDoc[key].filter((item: any) => {
                for (const [condKey, condValue] of Object.entries(
                  condition as any,
                )) {
                  if (item[condKey] === condValue) return false;
                }
                return true;
              });
            }
        ...
        if (update.$inc) {
          for (const [key, value] of Object.entries(update.$inc)) {
            if (typeof updatedDoc[key] === "number") {
              updatedDoc[key] += value as number;
            }
          }
        }
...
export const initializeSampleData = async () => {
  ...
  const chatGroupsCollection = mockDb.collection(
    "platform_patient_chat_groups",
  );
  ...
  const chatMessagesCollection = mockDb.collection(
    "platform_patient_chat_messages",
  );
  await chatMessagesCollection.insertMany([
    {
      ...
      message_type: "text",
      content:
        "Good morning! I'll be visiting Ahmed today at 9 AM for wound care. The healing progress has been excellent.",
      attachments: [],
    ...
    },
    {
      ...
      message_type: "text",
      content:
        "Thank you for the update! Dad is feeling much better today. Should we prepare anything special for the visit?",
      attachments: [],
  ...
  const emailCommunicationsCollection = mockDb.collection(
  ...
  await committeesCollection.insertMany([
    {
      ...
      description:
        "Oversees quality improvement initiatives and patient safety measures",
      members: [
        {
          user_id: "QM001",
    ...
    },
    {
      ...
      description:
        "Focuses on patient safety initiatives and incident prevention",
      members: [
        {
          user_id: "PS001",
        ...
        action_items: [
          {
            action_id: "ACT001",
            description:
              "Develop training materials for new wound care protocols",
  ...
  const governanceDocumentsCollection = mockDb.collection(
  ...
  await governanceDocumentsCollection.insertMany([
    {
      ...
      document_summary:
        "Comprehensive policy outlining standards for patient and family communication, including emergency notifications, care updates, and privacy considerations.",
      approval_workflow: {
    ...
    },
    {
      ...
      document_summary:
        "Step-by-step procedures for reporting, investigating, and managing patient safety incidents and near-miss events.",
      approval_workflow: {
        current_stage: "approved",
        approval_history: [
          {
  ...
  const staffAcknowledgmentsCollection = mockDb.collection(
  ...
  const communicationDashboardCollection = mockDb.collection(
  ...
  const communicationAnalyticsCollection = mockDb.collection(
