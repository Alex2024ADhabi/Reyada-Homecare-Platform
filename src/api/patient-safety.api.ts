...
router.post("/classify-incident", async (req, res) => {
  try {
    ...
    const classification = classifyIncidentByTaxonomy(
...
function calculateLevelOfHarm(incidentData) {
  ...
  if (incidentData.patient_harm === "none") return "B2";
  if (incidentData.patient_harm === "minor") return "E";
  if (incidentData.patient_harm === "moderate") return "F";
  if (incidentData.patient_harm === "severe") return "G";
  if (incidentData.patient_harm === "death") return "I";
