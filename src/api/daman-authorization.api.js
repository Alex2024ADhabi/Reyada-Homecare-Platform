    ...
    try {
        ...
        const enhancedResponse = {
            ...
            statusManagement: {
                ...
                nextSteps: getNextStepsBasedOnStatus(authorization.authorization_status),
                escalationOptions: {
                    available: authorization.authorization_status === "Denied" ||
                    ...
                    appealDeadline: authorization.authorization_status === "Denied"
                ...
                serviceDeliveryStatus: authorization.authorization_status === "Approved"
                    ? {
                        canBeginServices: true,
                        approvedStartDate: authorization.approval_effective_date ||
                            new Date(Date.now() + 86400000).toISOString(),
                        approvedEndDate: authorization.approval_expiration_date ||
                            new Date(Date.now() + 30 * 86400000).toISOString(),
            ...
            approvalDetails: authorization.authorization_status === "Approved"
                ? {
                    approvalCode: authorization.approval_code ||
                        `AUTH-${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
                    approvalDate: authorization.approval_date || new Date().toISOString(),
                    effectiveDate: authorization.approval_effective_date ||
                        new Date(Date.now() + 86400000).toISOString(),
                    expirationDate: authorization.approval_expiration_date ||
                        new Date(Date.now() + 30 * 86400000).toISOString(),
                    approvedBy: authorization.qa_review_assigned_to || "Dr. Fatima Al Zaabi",
                    approvedServices: authorization.requestedServices || [],
            ...
            rejectionDetails: authorization.authorization_status === "Denied"
                ? {
                    rejectionCode: authorization.rejection_code ||
                        `REJ-${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
                    rejectionDate: authorization.rejection_date || new Date().toISOString(),
                    rejectedBy: authorization.qa_review_assigned_to || "Dr. Fatima Al Zaabi",
                    ...
                    appealProcess: {
                        ...
                        appealInstructions: "Submit appeal with additional clinical documentation",
                        appealFormUrl: "https://provider.damanhealth.ae/appeals/form",
                    },
                    alternativeRecommendations: authorization.alternative_recommendations || [
...
router.get("/status/:referenceNumber", async (req, res) => {
    try {
        ...
        res.json({
            ...
            reviewTimeline: authorization.review_timeline || [],
            requiredAdditionalDocuments: authorization.required_additional_documents || [],
        });
    }
    catch (error) {
...
const submitDamanAuthorization = async (authorizationData) => {
    ...
    const complianceResult = await performRealTimeComplianceCheck(authorizationData);
...
const validateAuthorizationData = async (data) => {
    const errors = [];
...
const validateServiceCode = (serviceCode) => {
...
// MSC-specific validation requirements
const validateMSCRequirements = (data) => {
    const errors = [];
...
// Provider authentication validation
const validateProviderAuthentication = async (data) => {
    const errors = [];
...
// Real-time compliance checking
const performRealTimeComplianceCheck = async (data) => {
    const issues = [];
    ...
    if (data.wheelchairRequest) {
        ...
        if (currentDate >= wheelchairEffectiveDate) {
            if (!data.documents?.includes("Wheelchair Pre-approval Form")) {
                issues.push("Wheelchair pre-approval form is mandatory from May 1, 2025");
            }
            if (data.documents?.includes("PT Form") ||
                data.documents?.includes("Physiotherapy Form")) {
                issues.push("PT/Physiotherapy forms are no longer accepted for wheelchair requests");
            }
        }
    }
    ...
    if (data.homecareAllocation) {
        ...
        if (currentDate >= faceToFaceEffectiveDate) {
            if (!data.documents?.includes("Face-to-Face Assessment (OpenJet)")) {
                issues.push("Face-to-face assessment through OpenJet is mandatory from February 24, 2025");
            }
            if (!data.openJetIntegration) {
                issues.push("OpenJet system integration is required for automated homecare allocation");
            }
        }
    }
    return {
...
const logAuthorizationError = (error, context) => {
    const errorLog = {
    ...
    console.error("Daman Authorization Error:", JSON.stringify(errorLog, null, 2));
...
router.post("/submit", async (req, res) => {
    try {
        ...
        try {
            ...
            const missingFields = requiredFields.filter((field) => !sanitizedData[field] && sanitizedData[field] !== 0);
            ...
            if (sanitizedData.mscPlanExtension) {
                ...
                if (currentDate > mscDeadline) {
                    return res.status(400).json({
                        ...
                        details: "MSC plan extensions are no longer accepted after May 14, 2025",
                        complianceStatus: "failed",
                        dohStandards: "2025-non-compliant",
                        deadline: "2025-05-14",
                    });
            ...
            if (sanitizedData.wheelchairRequest) {
                ...
                const currentDate = new Date();
                if (currentDate >= wheelchairEffectiveDate) {
                    if (!sanitizedData.documents?.includes("Wheelchair Pre-approval Form")) {
                        return res.status(400).json({
                            error: "Missing wheelchair pre-approval form",
                            details: "Wheelchair pre-approval form is mandatory from May 1, 2025. PT forms are no longer accepted.",
                            complianceStatus: "failed",
                            dohStandards: "2025-non-compliant",
            ...
            if (sanitizedData.homecareAllocation) {
                ...
                const currentDate = new Date();
                if (currentDate >= faceToFaceEffectiveDate) {
                    if (!sanitizedData.documents?.includes("Face-to-Face Assessment (OpenJet)")) {
                        return res.status(400).json({
                            error: "Missing face-to-face assessment",
                            details: "Face-to-face assessment through OpenJet is mandatory from February 24, 2025",
                            complianceStatus: "failed",
                            dohStandards: "2025-non-compliant",
                    ...
                    if (!sanitizedData.openJetIntegration) {
                        return res.status(400).json({
                            ...
                            details: "OpenJet system integration is required for automated homecare allocation",
                            complianceStatus: "failed",
                            dohStandards: "2025-non-compliant",
                        });
                    }
        ...
        const referenceNumber = `DAMAN-PA-${new Date().getFullYear()}-${Math.floor(Math.random() * 100000)
            .toString()
            .padStart(5, "0")}`;
        ...
        const response = {
            ...
            submissionDate: validatedAuthorization.submission_date || new Date().toISOString(),
            estimatedReviewCompletion: validatedAuthorization.estimated_completion_date ||
        ...
        try {
            ...
            res.status(500).json({
                ...
                details: responseError instanceof Error
...
router.post("/:id/documents", async (req, res) => {
    try {
        ...
        const additionalDocuments = Array.isArray(authorization.additional_documents)
            ? [...authorization.additional_documents, ...files]
            : files;
        // Update the authorization status to "In Review" if it was "Additional Info Required"
        const newStatus = authorization.authorization_status === "Additional Info Required"
            ? "In Review"
        ...
        await db.collection("daman_authorizations").updateOne({ _id: new ObjectId(authorizationId) }, {
            $set: {
...
router.patch("/:id/status", async (req, res) => {
    try {
        ...
        // Update the authorization
        await db.collection("daman_authorizations").updateOne({ _id: new ObjectId(req.params.id) }, {
            $set: {
