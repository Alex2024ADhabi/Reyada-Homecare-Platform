...
export interface JsonFixOptions {
  autoFix: boolean;
...
export class ComprehensiveJsonValidator {
  ...
  static validateAndFix(
    ...
    let fixedJson: string | undefined;
   ...
   */
  static safeParse(jsonString: string): {
    success: boolean;
    data?: any;
    error?: string;
    warnings?: string[];
   ...
   */
  static validateComplaintData(complaintData: any): JsonValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    try {
      ...
      if (
        ...
        errors.push(
          "Invalid priority value - must be routine, urgent, or immediate",
        );
      }
      ...
      if (
        ...
        errors.push(
          "Invalid status value - must be received, acknowledged, investigating, resolved, or closed",
        );
      }
      ...
      if (
        ...
        errors.push(
          `Invalid complaint_type - must be one of: ${validComplaintTypes.join(", ")}`,
        );
      }
      ...
      if (complaintData.patient_contact) {
        if (
          !complaintData.patient_contact.phone &&
          !complaintData.patient_contact.email
        ) {
          errors.push(
            "Patient contact information incomplete - phone or email required for Tasneef audit",
          );
        }
        ...
        if (
          ...
          warnings.push(
            "Phone number should follow UAE format (+971XXXXXXXXX)",
          );
        }
        ...
        errors.push(
          "Patient contact information is required for Tasneef audit compliance",
        );
      }
      ...
      if (complaintData.description && complaintData.description.length < 20) {
        warnings.push(
          "Complaint description should be at least 20 characters for proper documentation",
        );
      }
      ...
      if (complaintData.sla_tracking) {
        if (
          !complaintData.sla_tracking.acknowledgment_sla ||
          !complaintData.sla_tracking.resolution_sla
        ) {
          errors.push(
            "SLA tracking requirements missing - acknowledgment and resolution SLA required",
          );
        }
      }
      ...
      if (
        ...
        errors.push(
          "Critical complaints must have immediate priority per Tasneef audit requirements",
        );
      }

      // Timeline compliance validation
      if (complaintData.daman_timeline_compliance) {
        const timelineValidation = this.validateDamanTimelineData(
          complaintData.daman_timeline_compliance,
        );
        if (!timelineValidation.isValid) {
          errors.push(
            ...timelineValidation.errors.map(
              (err) => `Timeline compliance: ${err}`,
            ),
          );
          warnings.push(
            ...timelineValidation.warnings.map(
              (warn) => `Timeline compliance: ${warn}`,
            ),
          );
        }
      }
      ...
      errors.push(
        `Complaint validation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
      return this.createResult(
        false,
        errors,
   ...
   */
  static validateDamanTimelineData(timelineData: any): JsonValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    try {
      ...
      if (
        ...
        errors.push(
          "Invalid submission_type - must be initial, renewal, extension, revision, or acute_care_notification",
        );
      }

      if (
        timelineData.submission_deadline &&
        !this.isValidDate(timelineData.submission_deadline)
      ) {
        errors.push(
          "Invalid submission_deadline format - must be valid ISO date",
        );
      }
      ...
      if (
        timelineData.submission_type === "extension" &&
        timelineData.days_remaining > 7
      ) {
        errors.push(
          "Extension requests must be submitted within 7 days - Tasneef audit requirement",
        );
      }

      // Enhanced revision timeline validation (30-day rule)
      if (
        timelineData.submission_type === "revision" &&
        timelineData.days_remaining > 30
      ) {
        errors.push(
          "Revision requests can only be accepted within 30 days - Tasneef audit requirement",
        );
      }

      // Acute care notification validation (10-day rule)
      if (
        timelineData.submission_type === "acute_care_notification" &&
        timelineData.days_remaining > 10
      ) {
        errors.push(
          "Acute care notifications must be submitted within 10 days - Tasneef audit requirement",
        );
      }

      // Prior approval requirement validation
      if (
        timelineData.submission_type === "initial" &&
      ...
      if (!timelineData.provider_responsibility_acknowledged) {
        warnings.push(
          "Provider responsibility must be acknowledged for full compliance",
        );
      }
      ...
      if (!timelineData.spc_guidelines_adherence) {
        errors.push(
          "SPC guidelines adherence is mandatory for Tasneef audit compliance",
        );
      }
  ...
  private static performBasicValidation(data: any): {
    isValid: boolean;
    errors: string[];
    warnings: string[];
  } {
    ...
    try {
      ...
      if (jsonString.length > this.MAX_SIZE) {
        warnings.push(
          `Data size (${jsonString.length}) exceeds recommended limit (${this.MAX_SIZE})`,
        );
      }
    } catch (error) {
    ...
    if (depth > this.MAX_DEPTH) {
      warnings.push(
        `Data depth (${depth}) exceeds recommended limit (${this.MAX_DEPTH})`,
      );
    }
  ...
  private static validateContentRecursive(
    data: any,
    options: JsonFixOptions,
    errors: string[],
    warnings: string[],
        ...
        Object.keys(data).forEach((key) => {
          ...
          if (options.sanitizeStrings && typeof data[key] === "string") {
            if (this.containsUnsafeContent(data[key])) {
              warnings.push(
                `Potentially unsafe content detected at ${newPath}`,
              );
            }
          }
  ...
  private static attemptAutoFix(
    data: any,
    options: JsonFixOptions,
  ): { success: boolean; fixedJson?: string; error?: string } {
    try {
  ...
  private static applyFixesRecursive(data: any, options: JsonFixOptions): void {
    if (typeof data === "object" && data !== null) {
      if (Array.isArray(data)) {
   ...
   */
  private static fixCommonJsonIssuesEnhanced(jsonString: string): {
    fixedString: string;
    fixedIssues: string[];
  } {
    let fixed = jsonString;
   ...
   */
  private static aggressiveJsonFix(jsonString: string): {
    success: boolean;
    data?: any;
    fixedIssues: string[];
  } {
  ...
  private static calculateDepth(obj: any, depth = 0): number {
    if (depth > this.MAX_DEPTH) return depth;
    ...
    if (Array.isArray(obj)) {
      ...
      for (const key in obj) {
        if (obj.hasOwnProperty(key)) {
          maxDepth = Math.max(
            maxDepth,
            this.calculateDepth(obj[key], depth + 1),
          );
        }
      }
    }
    ...
    if (typeof obj === "object" && obj !== null) {
      ...
      if (Array.isArray(obj)) {
        ...
        Object.values(obj).forEach((value) =>
          this.checkCircularReferences(value, seen, errors),
        );
      }
    }
  }
  ...
  private static containsUnsafeContent(str: string): boolean {
  ...
  private static fixDateFormat(dateString: string): string {
    try {
      const date = new Date(dateString);
...
export const validateComplaintData =
  ComprehensiveJsonValidator.validateComplaintData;
export const validateDamanTimelineData =
  ComprehensiveJsonValidator.validateDamanTimelineData;
