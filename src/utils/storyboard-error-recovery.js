...
export class StoryboardErrorRecovery {
     ...
     */
    static recordError(storyboardId, storyboardName, storyboardPath, error) {
     ...
     */
    static async attemptRecovery(storyboardId) {
        ...
        if (errorInfo.recoveryAttempts >= this.maxRecoveryAttempts) {
            console.warn(`Max recovery attempts reached for storyboard ${storyboardId}`);
            return this.createPermanentFallback(errorInfo);
        }
        console.log(`Attempting recovery for storyboard ${storyboardId} (attempt ${errorInfo.recoveryAttempts})`);
        // Try different recovery strategies
        for (const strategy of this.recoveryStrategies) {
            try {
                const recovered = await this.executeRecoveryStrategy(strategy, errorInfo);
                if (recovered) {
                    console.log(`✅ Recovery successful for ${storyboardId} using strategy: ${strategy}`);
                ...
                console.warn(`Recovery strategy ${strategy} failed for ${storyboardId}:`, recoveryError);
                continue;
            }
        }
     ...
     */
    static async executeRecoveryStrategy(strategy, errorInfo) {
        switch (strategy) {
     ...
     */
    static async reloadDependenciesStrategy(errorInfo) {
        try {
     ...
     */
    static async alternativeImportStrategy(errorInfo) {
        const alternativePaths = [
            errorInfo.path.replace("/home/peter/tempo-api/projects/4a0b90f3-3ca6-44b8-bc86-22f3300d4770/", "./"),
     ...
     */
    static createTemporaryFallback(errorInfo) {
        return function TemporaryStoryboardFallback() {
            return React.createElement("div", {
                ...
                React.createElement("div", {
                    ...
                    React.createElement("div", {
                        key: "details",
                        className: "text-xs text-orange-600 bg-orange-100 p-2 rounded",
                    }, [
                        React.createElement("div", { key: "path" }, `Path: ${errorInfo.path}`),
                        React.createElement("div", { key: "error" }, `Error: ${errorInfo.error.message}`),
                        React.createElement("div", { key: "time" }, `Time: ${errorInfo.timestamp.toLocaleString()}`),
                    ]),
                    React.createElement("div", {
                        key: "actions",
                        ...
                        React.createElement("button", {
                            ...
                            onClick: async () => {
                                const recovered = await StoryboardErrorRecovery.attemptRecovery(errorInfo.id);
                                if (recovered) {
     ...
     */
    static createPermanentFallback(errorInfo) {
            ...
            return React.createElement("div", {
                ...
                React.createElement("div", {
                    ...
                    React.createElement("div", {
                        ...
                        React.createElement("div", { key: "id" }, `ID: ${errorInfo.id}`),
                        React.createElement("div", { key: "path" }, `Path: ${errorInfo.path}`),
                        React.createElement("div", { key: "attempts" }, `Recovery Attempts: ${errorInfo.recoveryAttempts}`),
                        React.createElement("div", { key: "error" }, `Last Error: ${errorInfo.error.message}`),
                    ]),
                    React.createElement("button", {
                        key: "reload",
    ...
    static getErrorStats() {
        ...
        return {
            ...
            recoverable: errors.filter((e) => e.recoveryAttempts < this.maxRecoveryAttempts).length,
            permanent: errors.filter((e) => e.recoveryAttempts >= this.maxRecoveryAttempts).length,
            recentErrors: errors.filter((e) => e.timestamp > oneHourAgo).length,
        };
    }
}
Object.defineProperty(StoryboardErrorRecovery, "errors", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: new Map()
});
Object.defineProperty(StoryboardErrorRecovery, "maxRecoveryAttempts", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: 3
});
Object.defineProperty(StoryboardErrorRecovery, "recoveryStrategies", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: [
