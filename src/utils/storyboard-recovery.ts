...
export class StoryboardRecovery {
   ...
   */
  static async recoverStoryboard(
    ...
    for (let i = 0; i < opts.maxRetries; i++) {
      ...
      try {
        ...
        if (component && typeof component === "function") {
          if (opts.logErrors) {
            console.log(
              `✅ Storyboard recovered on attempt ${attempts}: ${storyboardName}`,
            );
          }
        ...
        if (opts.logErrors) {
          console.warn(
            `Attempt ${attempts} failed for ${storyboardName}:`,
            error,
          );
        }
    ...
    for (const altPath of alternativePaths) {
      ...
      try {
        ...
        if (component && typeof component === "function") {
          if (opts.logErrors) {
            console.log(
              `✅ Storyboard recovered with alternative path: ${altPath}`,
            );
          }
          return { success: true, component, attempts };
        }
    ...
    if (opts.fallbackComponent) {
      const fallbackComponent = this.createFallbackComponent(
        storyboardName,
        lastError,
      );
      return {
        ...
        [
          ...
          React.createElement(
            "button",
            {
              key: "reload",
              className:
                "mt-4 px-4 py-2 bg-yellow-600 text-white rounded hover:bg-yellow-700",
