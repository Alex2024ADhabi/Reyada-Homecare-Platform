...
export class StoryboardDiagnostics {
   ...
   */
  static async runDiagnostics(): Promise<SystemDiagnostics> {
  ...
  private static checkReactAvailability(): DiagnosticResult {
    try {
      ...
      if (!React) {
        return {
          ...
          details: { windowReact: !!(window as any).React, globalReact: !!(globalThis as any).React }
        };
      }
  ...
  private static checkDynamicImportSupport(): DiagnosticResult {
    try {
  ...
  private static checkViteEnvironment(): DiagnosticResult {
    try {
  ...
  private static checkMemoryUsage(): DiagnosticResult {
    try {
  ...
  private static checkNetworkConnectivity(): DiagnosticResult {
    try {
      if (!navigator.onLine) {
        return {
  ...
  private static checkModuleResolution(): DiagnosticResult {
    try {
   ...
   */
  static formatDiagnosticReport(diagnostics: SystemDiagnostics): string {
