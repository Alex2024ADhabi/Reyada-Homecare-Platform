...
export class StoryboardDependencyResolver {
  private static resolvedDependencies = new Map<string, any>();
  private static failedDependencies = new Set<string>();

  /**
   * Pre-resolve common dependencies to prevent runtime failures
   */
  static async preResolveDependencies(): Promise<void> {
    ...
    console.log(
      `🎯 Dependency resolution complete: ${this.resolvedDependencies.size} resolved, ${this.failedDependencies.size} failed`,
    );
  }
   ...
   */
  static getResolvedDependency(depName: string): any {
   ...
   */
  static isDependencyFailed(depName: string): boolean {
   ...
   */
  static createSafeStoryboardWrapper(
    storyboardComponent: any,
    storyboardName: string,
  ): React.ComponentType {
    return function SafeStoryboardWrapper(props: any) {
      try {
        ...
        return React.createElement(
          ...
          {
            className:
              "p-8 bg-red-50 border border-red-200 rounded-lg text-center",
          },
          [
            React.createElement(
              "h3",
              ...
              [
                ...
                React.createElement(
                  "pre",
                  {
                    key: "error",
                    className:
                      "mt-2 p-2 bg-red-100 rounded text-xs overflow-auto",
                  },
                  error.toString(),
                ),
              ],
   ...
   */
  static async loadStoryboardWithDependencies(
    ...
    try {
      ...
      return this.createSafeStoryboardWrapper(
        storyboardComponent,
        storyboardName,
      );
    } catch (error) {
      console.error(
        `Failed to load storyboard ${storyboardName} from ${storyboardPath}:`,
        error,
      );
