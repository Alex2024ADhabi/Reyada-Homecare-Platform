...
export async function safeDynamicImport(modulePath) {
    try {
        ...
        try {
            ...
            console.warn(`Alternative import also failed for ${modulePath}:`, altError);
            return null;
        }
    }
}
// Safe conditional import for tempo routes with bulletproof fallback
export async function loadTempoRoutes() {
    ...
    try {
        ...
        if (typeof window !== "undefined" && window.__TEMPO_ROUTES__) {
            console.log("✅ Using cached tempo routes");
            return window.__TEMPO_ROUTES__;
        }
        ...
        try {
            ...
            tempoModule = await Promise.race([
                import("tempo-routes"),
                new Promise((_, reject) => setTimeout(() => reject(new Error("Import timeout")), importTimeout)),
            ]);
        }
        catch (directError) {
            console.warn("Direct tempo-routes import failed, trying alternative:", directError);
            try {
                // Try dynamic import with vite-ignore and timeout
                tempoModule = await Promise.race([
                    import(/* @vite-ignore */ "tempo-routes"),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Dynamic import timeout")), importTimeout)),
                ]);
            }
            catch (dynamicError) {
        ...
        if (typeof window !== "undefined" && routesArray.length > 0) {
            window.__TEMPO_ROUTES__ = routesArray;
            window.__TEMPO_ROUTES_TIMESTAMP__ = Date.now();
        }
...
export async function loadService(servicePath) {
    try {
        ...
        try {
            ...
            console.warn(`Alternative service loading failed for ${servicePath}:`, altError);
            return null;
        }
    }
}
// Enhanced storyboard loading with dependency resolution
export async function loadStoryboard(storyboardPath) {
    try {
        // Network connectivity check
        if (!navigator.onLine) {
            console.warn(`Network offline - cannot load storyboard: ${storyboardPath}`);
        ...
        try {
            storyboard = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error("Storyboard import timeout")), loadTimeout)),
            ]);
        }
        catch (directError) {
            console.warn(`Direct storyboard import failed for ${storyboardPath}:`, directError);
            ...
            for (const altPath of alternativePaths) {
                try {
                    storyboard = await Promise.race([
                        ...
                        new Promise((_, reject) => setTimeout(() => reject(new Error("Alternative path timeout")), 5000)),
                    ]);
                    if (storyboard) {
                        console.log(`Successfully loaded storyboard from alternative path: ${altPath}`);
                        break;
                    }
                }
                catch (altError) {
...
function createNetworkErrorFallback(storyboardPath) {
...
function createGeneralErrorFallback(storyboardPath, error) {
